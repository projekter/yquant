...
% Loop #1 from #2 to #3 (#2 <= #3), executing #4, with no loop if they are not in order.
% Starred version: Loop #1 from min(#2, #3) to max(#2, #3), executing #4
\protected\def\yquant@for{%
   \@ifstar{\yquant@for@\yquant@for@minmax}{\yquant@for@\yquant@for@inorder}%
}

\def\yquant@for@#1#2:=#3to#4#{#1#2{#3}{#4}}

% Loop #1 from #2 down to #3 (#2 >= #3), executing #4, with no loop if they are not in reverse order.
% Starred version: Loop #1 from max(#2, #3) down to min(#2, #3), executing #4
\protected\def\yquant@fordown{%
   \@ifstar{\yquant@fordown@\yquant@fordown@minmax}{\yquant@fordown@\yquant@fordown@inorder}%
}

\def\yquant@fordown@#1#2:=#3downto#4#{#1#2{#3}{#4}}

\def\yquant@for@inorder#1#2#3{%
   \ifyquant\ifnum#2>#3 \fi\@gobble{%
      \numdef#1{#2}%
      \expandafter\yquant@for@loop\expandafter#1\expandafter{\the\numexpr#3+1\relax}%
   }%
}

\def\yquant@for@minmax#1#2#3{%
   \ifyquant\ifnum#2<#3 \fi{%
      \numdef#1{#2}%
...
\def\yquant@fordown@inorder#1#2#3{%
   \ifyquant\ifnum#3>#2 \fi\@gobble{%
      \numdef#1{#2}%
      \expandafter\yquant@fordown@loop\expandafter#1\expandafter{\the\numexpr#3-1\relax}%
   }%
}

\def\yquant@fordown@minmax#1#2#3{%
   \ifyquant\ifnum#2>#3 \fi{%
      \numdef#1{#2}%
      % to allow for things like \yquant@for \i := \i to ..., expand the boundaries
      \expandafter\yquant@fordown@loop\expandafter#1\expandafter{\the\numexpr#3-1\relax}%
...
% We don't use \loop...\repeat from the kernel, as this cannot be nested (without grouping). Our definition is almost the same, but we don't take a static \iterate macro which is always overwritten, but take a macro name. The loops calculate this name based on the loop variable.
\long\def\yquant@for@@loop#1#2\yquant@for@@repeat{%
   \def#1{%
      #2%
      \relax%
      \expandafter#1%
...
\long\def\yquant@for@loop#1#2#3{%
   \expandafter\yquant@for@@loop\csname yquant@forloop@iterate@\string#1\endcsname%
      \ifnum#1<#2 %
         #3%
         \numdef#1{#1+1}%
   \yquant@for@@repeat%
...
\protected\def\yquant@sort@findfirst#1#2{%
   ...
   \else%
      ...
      \yquant@counta=1 %
      \loop%
         \ifnum\yquant@counta<\yquant@sort@count\relax%
            \expandafter\expandafter\expandafter\expandafter\expandafter\expandafter\expandafter#1%
               \expandafter\expandafter\expandafter\expandafter\expandafter\expandafter\expandafter{%
               \expandafter\expandafter\expandafter#2%
               \expandafter\expandafter\expandafter}%
               \expandafter\expandafter\expandafter{%
                  \csname yquant@sort@item\the\yquant@counta\endcsname%
               }%
               \relax{%
               \letcs#2{yquant@sort@item\the\yquant@counta}%
            }%
            \advance\yquant@counta by 1 %
      \repeat%
   \fi%
}
...
\protected\def\yquant@sort@aux#1#2{%
   \ifnum#1<#2\relax%
      ...
      \edef\@yquant@cmd{%
         \noexpand\yquant@sort@aux{#1}{\the\numexpr\yquant@counta-1\relax}%
         \noexpand\yquant@sort@aux{\the\numexpr\yquant@counta+1\relax}{#2}%
      }%
      \@yquant@cmd%
   \fi%
}

\def\yquant@iftrue{\iftrue}%
\def\yquant@iffalse{\iffalse}%
\protected\def\yquant@sort@divide#1#2{%
   \yquant@counta=#1\relax% i
   \yquant@countb=#2\relax% j
   \advance\yquant@countb by -1 %
   ...
   \loop%
            ...
            \ifnum\yquant@counta<#2\relax%
               \expandafter\expandafter\expandafter\expandafter\expandafter\expandafter\expandafter\yquant@sort@cmp%
                  ...
                  \expandafter\expandafter\expandafter{%
                     ...
                     \expandafter\yquant@iffalse%
                  }{%
                     \advance\yquant@counta by 1 %
                     \expandafter\yquant@iftrue%
                  }%
            \else%
               \expandafter\yquant@iffalse%
            \fi%
      ...
      \expandafter\yquant@counta\expandafter=\the\yquant@counta\relax%
      % search an item from the right that is small than the pivot
      {% protect the outer loop from finding \repeat
         \loop%
            \ifnum\yquant@countb>#1\relax%
               \expandafter\expandafter\expandafter\expandafter\expandafter\expandafter\expandafter\yquant@sort@cmp%
                  \expandafter\expandafter\expandafter\expandafter\expandafter\expandafter\expandafter{%
                  \expandafter\expandafter\expandafter\yquant@sort@pivot%
                  \expandafter\expandafter\expandafter}%
                  \expandafter\expandafter\expandafter{%
                     \csname yquant@sort@item\the\yquant@countb\endcsname%
                  }{%
                     \advance\yquant@countb by -1 %
                     \expandafter\yquant@iftrue%
                  }{%
      ...
      \expandafter\yquant@countb\expandafter=\the\yquant@countb\relax%
      \ifnum\yquant@counta<\yquant@countb %
         % swap item i <> item j
         \letcs\@yquant@tmp{yquant@sort@item\the\yquant@counta}%
         \csletcs{yquant@sort@item\the\yquant@counta}{yquant@sort@item\the\yquant@countb}%
         \cslet{yquant@sort@item\the\yquant@countb}\@yquant@tmp%
      \fi%
      \ifnum\yquant@counta<\yquant@countb %
   \repeat%
   \letcs\@yquant@tmp{yquant@sort@item\the\yquant@counta}%
   \csletcs{yquant@sort@item\the\yquant@counta}{yquant@sort@item#2}%
   \cslet{yquant@sort@item#2}\@yquant@tmp%
}
...
\protected\def\yquant@sort@list#1#2{%
   \begingroup%
      ...
      \yquant@counta=0 %
      \loop%
         \ifnum\yquant@counta<\yquant@sort@count%
...
\protected\def\yquant@sort@dolistloop{%
   ...
   \loop%
      \ifnum\yquant@counta<\yquant@sort@count%
         \expandafter\expandafter\expandafter\@yquant@do%
         \expandafter\expandafter\expandafter{%
            \csname yquant@sort@item\the\yquant@counta\endcsname%
         }%
         \advance\yquant@counta by 1 %
...
\protected\def\ifyquant@registersubset#1#2{%
   \begingroup%
      ...
      \edef\@yquant@listA{#1}%
      \edef\@yquant@listB{#2}%
      \let\@ifyquant@success=\iftrue%
      \forlistloop\yquant@registersubset@loop\@yquant@listA%
      \expandafter%
   \endgroup%
   \@ifyquant@success%
      \expandafter\@firstoftwo%
...
\protected\def\yquant@registersubset@loop#1{%
   \ifinlist{#1}\@yquant@listB\relax{%
      \let\@ifyquant@success=\iffalse%
      \listbreak%
   }%
}
...
% #1 is a pgf soft path. We extract the maximum x position at the y position specified in #2 and assign it to \yquant@dimena, which is translated to the user coordinate system.
...
\protected\def\yquant@softpath@extractmaxxat#1#2{%
   \begingroup%
      ...
      \expandafter%
   \endgroup%
   \expandafter\yquant@dimena\expandafter=\the\dimen0\relax%
}
