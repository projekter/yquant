...
\protected\def\yquant@draw@move#1#2{%
   \begingroup%
      \ifcsname\yquant@prefix xshift\endcsname%
         \dimen0=\dimexpr#2+\csname\yquant@prefix xshift\endcsname\relax%
      \else%
         \dimen0=#2 %
      \fi%
      \let\pgfshapeclippathhorzresult=\empty%
...
\newif\ifyquant@draw@callback@now
   ...
   \def\do##1{%
      ...
      \ifyquant@firsttoken\yquant@register@multi{##1}{%
         ...
         \ifx\yquant@draw@@multi\yquant@draw@@multiinit%
            % if we draw an initialization at the beginning (whether multi or single), this possibly affects the minimal x position. All other gates will be shifted so that they cannot extend beyond the minimal position.
            \ifdim\pgf@picminx<\csname\yquant@prefix xmin\endcsname%
               \csxdef{\yquant@prefix xmin}{\the\pgf@picminx}%
            \fi%
         \fi%
         \ifx\yquant@draw@@multi\yquant@draw@@output@multi%
            % if we draw an output gate, appropriately adjust the total width
...
\protected\long\def\yquant@draw@@finalize#1#2{%
   ...
   \protected@edef\yquant@draw@append{%
      \noexpand\yquant@draw@group%
         {\ifx\yquant@draw@@multi\yquant@draw@@output@multi%
            \yquant@env@end@xpos% end of circuit
          \else%
...
\protected\long\def\yquant@draw@@output@single#1#2#3{%
   \yquant@sort@eadd{%
   ...
   \pgfinterruptboundingbox%
      ...
      % #2 is empty anyway
      \edef\cmd{%
         \noexpand\path (\yquant@env@end@xpos, 0pt)%
...
\protected\long\def\yquant@draw@@output@multi#1#2#3{%
   \yquant@sort@eadd{%
   ...
   \pgfinterruptboundingbox%
      ...
      % #2 is empty anyway
      \edef\cmd{%
         \noexpand\path (0pt, \yquant@env@end@xpos)
            node[\ifnum\yquant@compat>1 /yquant/every operator, /yquant/every output, \fi%
...
\protected\def\yquant@draw@@subcircuit@single#1#2{%
   ...
   \expandafter\yquant@circuit@subcircuit{#1}%
   \yquant@sort@eadd{%
...
\protected\def\yquant@draw@@finalize@ctrl@multiinit#1#2#3#4#5{%
   {%
      \yquant@for \yquant@i := #1 to #2 {%
         \yquant@register@set@x\yquant@i{1sp}%
      }%
   }%
   ...
   \eappto\yquant@draw@append{\csname\yquant@prefix finalize@ctrl@draw@appto\endcsname}%
   \ifyquant@draw@callback@now%
      \csname\yquant@prefix finalize@ctrl@draw@appto\endcsname%
   \fi%
   \csgundef{\yquant@prefix finalize@ctrl@draw@appto}%
}
