...
\protected\def\yquant@draw@init#1#2{%
   \ifcsname\yquant@prefix registermap@#1\endcsname%
      % for connected subcircuit wires, check if this actually changes the type and flush the wire if necessary
...
\protected\long\def\yquant@draw@group#1#2#3#4#5#6{%
   \begingroup%
      ...
      \ifyquant@config@operator@position@rightalign%
         % for rightalign, we instead overwrite the lastx configurations, so that the outgoing wire starts after the operator. The wire should have been flushed before.
...
\protected\long\def\yquant@draw@subcircuit@prepare#1#2#3#4{%
   ...
   \pgfinterruptboundingbox%
      ...
      \cmd%
      \edef\restoredims{%
         \noexpand\ifdim\pgf@picminx>\the\pgf@picminx%
            \global\pgf@picminx=\the\pgf@picminx\space%
         \noexpand\fi%
         \noexpand\ifdim\pgf@picmaxx<\the\pgf@picmaxx%
            \global\pgf@picmaxx=\the\pgf@picmaxx\space%
         \noexpand\fi%
         \noexpand\ifdim\pgf@picminy>\the\pgf@picminy%
            \global\pgf@picminy=\the\pgf@picminy\space%
         \noexpand\fi%
         \noexpand\ifdim\pgf@picmaxy<\the\pgf@picmaxy%
            \global\pgf@picmaxy=\the\pgf@picmaxy\space%
         \noexpand\fi%
      }%
      \expandafter%
