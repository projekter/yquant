...
\protected\def\yquant@draw@multi@contiguous#1#2#3{%
   ...
   \yquant@for \@yquant@i := #1 to #2 {%
      \yquant@circuit@extendwire\@yquant@i{center}%
   }%
...
\protected\long\def\yquant@draw@subcircuit@prepare#1#2#3#4#5{%
   ...
   \yquant@begingroup%
      ...
      \edef\pgf@interrupt@savemaxx{\the\yquant@pgf@picmaxx}%
      \edef\pgf@interrupt@saveminx{\the\yquant@pgf@picminx}%
      \yquant@pgf@picmaxy=-16000pt %
      \yquant@pgf@picminy=16000pt %
      \yquant@pgf@picmaxx=-16000pt %
      \yquant@pgf@picminx=16000pt %
      ...
      \yquant@dimena=\@yquant@newx\space%
      \let\yquant@draw@subcircuit@leftpos=\@yquant@newx% seamless subcircuits: do not add another separation if we are there
      ...
      \setbox\yquant@prepare@subcircuit@box=\hbox{{%
         \let\yquant@draw@subcircuit@wirestart=\@yquant@newx%
         ...
         \ifpgf@pt@identity%
            \yquant@dimena=\dimexpr\yquant@register@get@y1+\yquant@pgf@pt@y\relax%
            \ifdim\yquant@dimena>\yquant@pgf@picmaxy %
               \global\yquant@pgf@picmaxy=\yquant@dimena%
            \fi%
            \yquant@dimena=\dimexpr\yquant@register@get@y{\csname\yquant@prefix registers\endcsname}+\yquant@pgf@pt@y\relax%
            \ifdim\yquant@dimena<\yquant@pgf@picminy %
               \global\yquant@pgf@picminy=\yquant@dimena%
            \fi%
         \else%
            % first wire, left end
      ...
      \ifpgf@pt@identity%
         \yquant@dimena=\dimexpr.5\yquant@pgf@picminy+.5\yquant@pgf@picmaxy-\yquant@pgf@pt@y\relax%
         \yquant@dimenb=\dimexpr\yquant@pgf@picmaxy-\yquant@pgf@picminy\relax%
      \else%
         \begingroup%
            \ifdim\pgf@pt@ab pt=0pt% may also be reflection
               ...
               \yquant@dimenb=\dimexpr\yquant@pgf@y-\@tempdima\relax%
               \ifdim\yquant@dimenb<0pt % reflections
                  \yquant@dimenb=-\yquant@dimenb%
               \fi%
               \yquant@dimena=\dimexpr.5\dimexpr\@tempdima+\yquant@pgf@y\relax\relax%
            \else%
               ...
               \dimen\ifyquanthorz24=\dimexpr\dimexpr\pgf@pt@aa\@tempdimb-\pgf@pt@ab\@tempdima\relax*65536/%
                                             \@tempdimc\relax%
               \dimen\ifyquanthorz42=\dimexpr\dimexpr\pgf@pt@aa\@tempdima-\pgf@pt@ab\@tempdimb\relax*65536/%
                                             \@tempdimc\relax%
               % The y position of the middle point is simply .5\pgf@picminy+.5\pgf@picmaxy in the unrotated system. Hence, we have to calculate the full point (x, y) and rotate it backwards:
               \ifyquanthorz{%
                  % y cos alpha - x sin alpha
                  \yquant@dimena=.5\dimexpr\pgf@pt@aa\dimexpr\pgf@picminy+\pgf@picmaxy-2\pgf@pt@y\relax-\pgf@pt@ab\dimexpr\pgf@picminx+\pgf@picmaxx-2\pgf@pt@x\relax\relax%
               }{%
               % x cos alpha + y sin alpha
                  \yquant@dimena=.5\dimexpr\pgf@pt@aa\dimexpr\pgf@picminx+\pgf@picmaxx-2\pgf@pt@x\relax+\pgf@pt@ab\dimexpr\pgf@picminy+\pgf@picmaxy-2\pgf@pt@y\relax\relax%
               }%
               \ifdim\yquant@dimenb<0pt % reflections
                  \yquant@dimenb=-\yquant@dimenb%
               \fi%
            ...
            \edef\@yquant@cmd{%
               \endgroup%
               \yquant@dimena=\the\yquant@dimena%
               \yquant@dimenb=\the\yquant@dimenb%
            }%
         \@yquant@cmd%
      \fi%
      % now, due to cross-subcircuit alignments of enclosing rectangles, it might be that our computed width is far larger than the one we would expect
      \yquant@dimenc=\yquant@orientation@minus\maxdimen %
      \forlistloop\yquant@draw@subcircuit@remeasure{#5}%
      \advance \yquant@dimenc by \yquant@orientation@plus\yquant@circuit@endwires@operatorsep\space%
      \gundef\yquant@circuit@endwires@operatorsep%
      \yquant@dimend=\@firstofthree#4 %
      \dimdef\@yquant@delta{%
         \yquant@dimenc%
         -\yquant@draw@@x%
         \yquant@orientation@minus.5\yquant@dimend%
      }%
      \ifdim\dimexpr\@yquant@delta\yquant@orientation@minus100sp\relax\ifyquanthorz><0pt %
         % we simply increase our size - but this cannot help for the centering if there are multiple subcircuits or a conditional line
         \dimdef\yquant@draw@@x{\yquant@draw@@x+.5\dimexpr\@yquant@delta\relax}%
         \dimdef\yquant@draw@@width{\yquant@draw@@width\yquant@orientation@plus\@yquant@delta}%
         \advance \yquant@dimend by \yquant@orientation@plus\@yquant@delta\space%
      \fi%
      \edef\@yquant@cmd{%
         \noexpand\path \yquant@coords(\yquant@draw@@x, \the\yquant@dimena)%
            node[/yquant/every operator, \yquant@draw@@style,%
                 ...
                 name prefix=, name suffix=, name=yquantbox]%
            {\ifyquanthorz{\vbox to \the\yquant@dimenb {\hbox to \the\yquant@dimend {}}}%
                          {\hbox to \the\yquant@dimenb {\vbox to \the\yquant@dimend {}}}};%
      }%
      \ifyquant@config@draw@quality%
         \@yquant@cmd%
      \else%
   ...
   % \endpgfinterruptboundingbox + increase
      ...
      \fi%
      \ifdim\pgf@interrupt@savemaxx>\yquant@pgf@picmaxx%
         \global\yquant@pgf@picmaxx=\pgf@interrupt@savemaxx%
      \fi%
      \ifdim\pgf@interrupt@saveminx<\yquant@pgf@picminx%
         \global\yquant@pgf@picminx=\pgf@interrupt@saveminx%
      \fi%
      % Now that the subcircuit is finished, we need advance all the wires
      \dimdef\@yquant@newx{\yquant@draw@@x\yquant@orientation@plus.5\dimexpr\yquant@draw@@width\relax}%
      \forlistloop\yquant@draw@group@advance{#5}%
   \yquant@endgroup%
   % name mangling
   \ifcase\yquant@config@operator@subcircuit@mangling\relax%
      ...
      \ifstrempty{#2}{%
         ...
         \forlistcsloop\@yquant@do{\yquant@draw@subcircuit@nodelist}%
...
\protected\def\yquant@draw@subcircuit@remeasure#1{%
   \ifdim\yquant@dimenc\ifyquanthorz<>\yquant@register@get@x{#1} %
      \yquant@dimenc=\yquant@register@get@x{#1} %
   \fi%
}
      ...
      \yquant@for \@yquant@i := #1 to #2 {%
         \xifinlist{\@yquant@i}{#4}{%
            ...
            \yquant@softpath@extractmaxxat\nonaffectedpgfshapeclippathhorzresult%
                                          {\yquant@register@get@y\@yquant@i}%
            \let\pgfshapeclippathhorzresult=\empty%
            \yquant@circuit@extendwire\@yquant@i*%
         }{%
...
\protected\def\yquant@draw@wire#1#2{%
   \yquant@begingroup%
      \yquant@register@get@typeywire{#1}\@yquant@wiretype\@yquant@wireypos\@yquant@wirelast%
      \unless\ifnum\@yquant@wiretype=\yquant@register@types@nobit%
         \edef\@yquant@wirexprevpos{\expandafter\@firstoffour\@yquant@wirelast}%
         \ifx0#2%
            \edef\@yquant@wirexpos{\expandafter\@secondoffour\@yquant@wirelast}%
         \else%
            \let\@yquant@wirexpos=\yquant@circuit@endwires@x%
         \fi%
         \ifdim\yquant@orientation@plus\@yquant@wirexpos>\yquant@orientation@plus\@yquant@wirexprevpos %
            \edef\@yquant@wirestyle{\noexpand\tikzset{%
               /yquant/this wire/.style={%
                  /yquant/every wire,%
                  /yquant/every \yquant@register@type@tostring\@yquant@wiretype\space wire,%
            ...
            \ifyquant@config@draw@quality%
               % load all clippings
               \edef\@yquant@wireclipping{%
                  \unexpanded\expandafter\expandafter\expandafter{%
                     \expandafter\@thirdandfourthoffour\@yquant@wirelast%
                  }%
               }%
               \pgfscope%
                  \unless\ifx\@yquant@wireclipping\empty% may happen if the style is altered to be invisible, though the type is not none
                     % install the clipping
                     \pgfsyssoftpath@setcurrentpath\@yquant@wireclipping%
                     % invert the clipping
                     \ifyquantdebug%
                  ...
                  \csname yquant@draw@wire@\@yquant@wiretype\endcsname{#1}%
               \endpgfscope%
            \else%
               \yquant@draw@onwires{%
                  \csname yquant@draw@wire@\@yquant@wiretype\endcsname{#1}%
...
\protected\csdef{yquant@draw@wire@\yquant@register@types@qubit}#1{%
   \edef\@yquant@cmd{%
      \noexpand\path [/yquant/this wire]%
         \yquant@coords(\@yquant@wirexprevpos,\@yquant@wireypos) -- \yquant@coords(\@yquant@wirexpos,\@yquant@wireypos);%
   }%
   \@yquant@cmd%
}
...
\protected\csdef{yquant@draw@wire@\yquant@register@types@cbit}#1{%
   \edef\@yquant@cmd{%
      \noexpand\path [/yquant/this wire]%
         \yquant@coords(\@yquant@wirexprevpos,\@yquant@wireypos+2\pgflinewidth) --
         \yquant@coords(\@yquant@wirexpos,\@yquant@wireypos+2\pgflinewidth)%
         \yquant@coords(\@yquant@wirexprevpos,\@yquant@wireypos-2\pgflinewidth) --
         \yquant@coords(\@yquant@wirexpos,\@yquant@wireypos-2\pgflinewidth);%
   }%
   \@yquant@cmd%
...
\protected\csdef{yquant@draw@wire@\yquant@register@types@qubits}#1{%
   \edef\@yquant@cmd{%
      \noexpand\path [/yquant/this wire]%
         \yquant@coords(\@yquant@wirexprevpos,\@yquant@wireypos+2\pgflinewidth) --
         \yquant@coords(\@yquant@wirexpos,\@yquant@wireypos+2\pgflinewidth)%
         \yquant@coords(\@yquant@wirexprevpos,\@yquant@wireypos) --
         \yquant@coords(\@yquant@wirexpos,\@yquant@wireypos)%
         \yquant@coords(\@yquant@wirexprevpos,\@yquant@wireypos-2\pgflinewidth) --
         \yquant@coords(\@yquant@wirexpos,\@yquant@wireypos-2\pgflinewidth);%
   }%
   \@yquant@cmd%
...
\protected\def\yquant@draw@hspace#1#2{%
   \begingroup%
      \yquant@register@get@maxxlist\@yquant@newx{#1}%
      \dimdef\@yquant@newx{\@yquant@newx\yquant@orientation@plus#2}%
      \yquant@dimena=\@yquant@newx\space%
      \let\pgfshapeclippathhorzresult=\empty%
...
\protected\def\yquant@draw@enclose@measure@anyrelevant#1#2{%
   \ifyquant\ifcsname yquant@circuit@enclose@#1@extentT@\yquant@register@resolvetocolon{#2}\endcsname\fi{%
      \let\@yquant@next=\@firstofone%
      \listbreak%
   }\relax%
}

\protected\def\yquant@draw@enclose@measure@register#1#2{%
   \edef\@yquant@resolution{\yquant@register@resolvetocolon{#2}}%
   \ifcsname yquant@circuit@enclose@#1@extentT@\@yquant@resolution\endcsname%
      \ifcsname yquant@draw@enclose@#1@left@\@yquant@resolution\endcsname%
         \ifdim\csname yquant@draw@enclose@#1@left@\@yquant@resolution\endcsname>\@yquant@leftpos\space%
            \global\cslet{yquant@draw@enclose@#1@left@\@yquant@resolution}\@yquant@leftpos%
         \fi%
         \ifdim\csname yquant@draw@enclose@#1@right@\@yquant@resolution\endcsname<\@yquant@rightpos\space%
            \global\cslet{yquant@draw@enclose@#1@right@\@yquant@resolution}\@yquant@rightpos%
         \fi%
      \else%
         \global\cslet{yquant@draw@enclose@#1@left@\@yquant@resolution}\@yquant@leftpos%
         \global\cslet{yquant@draw@enclose@#1@right@\@yquant@resolution}\@yquant@rightpos%
   ...
   \begingroup%
      \ifcase\csname yquant@circuit@enclose@#2@shape\endcsname\space%
         ...
         \expandafter\ifx\csname yquant@circuit@enclose@#2@involved\endcsname\yquant@circuit@enclose@allinvolved%
            \let\@yquant@next=\@firstofone%
         \else%
            \let\@yquant@next=\@gobble%
            \forlistloop{\yquant@draw@enclose@measure@anyrelevant{#2}}{#1}%
         \fi%
         \@yquant@next{%
            \edef\@yquant@width{\unless\ifyquant@config@operator@position@rightalign.5\fi\dimexpr\yquant@draw@@width\relax}%
            \ifdim\csname yquant@draw@enclose@#2@left\endcsname>\dimexpr\yquant@draw@@x-\@yquant@width\relax%
               \csdimgdef{yquant@draw@enclose@#2@left}{\yquant@draw@@x-\@yquant@width}%
            \fi%
            \ifdim\csname yquant@draw@enclose@#2@right\endcsname<\dimexpr\yquant@draw@@x+\@yquant@width\relax%
               \csdimgdef{yquant@draw@enclose@#2@right}{\yquant@draw@@x+\@yquant@width}%
            \fi%
         }%
      \or%
         \edef\@yquant@width{\unless\ifyquant@config@operator@position@rightalign.5\fi\dimexpr\yquant@draw@@width\relax}%
         \dimdef\@yquant@leftpos{\yquant@draw@@x-\@yquant@width}%
         \dimdef\@yquant@rightpos{\yquant@draw@@x+\@yquant@width}%
...
\protected\def\yquant@draw@stopenclose#1{%
   ...
   \begingroup%
      \begingroup%
         ...
         \tikz@enable@node@quotes%
         \tikzset{/yquant/enclose/every enclose}%
         ...
         \edef\@yquant@setsep{%
            \endgroup%
            \ifyquanthorz\yquant@dimenc\yquant@dimend=\the\pgf@x\space%
            \ifyquanthorz\yquant@dimend\yquant@dimenc=\the\pgf@y\space%
         }%
         \@yquant@setsep%
      \expandafter\ifx\csname yquant@circuit@enclose@#1@involved\endcsname\yquant@circuit@enclose@allinvolved%
         \def\@yquant@i{1}%
      ...
      \else%
         ...
         \numdef\@yquant@i{\yquant@sort@count-1}%
      \fi%
      \yquant@draw@enclose@calcbottom{#1}\@yquant@i%
      \yquant@dimena=\@yquant@enclosebottom\space% store the bottom-most position
      \ifcase\csname yquant@circuit@enclose@#1@shape\endcsname\space%
         ...
         \fi%
         \dimdef\@yquant@encloseleft{\csname yquant@draw@enclose@#1@left\endcsname-\yquant@dimenc}%
         \yquant@dimenb=\@yquant@encloseleft\space%
         \dimdef\@yquant@encloseright{\csname yquant@draw@enclose@#1@right\endcsname+\yquant@dimenc}%
         \yquant@draw@enclose@calctop{#1}0%
         \edef\@yquant@points{%
            \yquant@coords(\@yquant@encloseleft,\@yquant@enclosebottom)%
            \yquant@coords(\@yquant@encloseright,\@yquant@enclosebottom)%
            \yquant@coords(\@yquant@encloseright,\@yquant@enclosetop)%
            \yquant@coords(\@yquant@encloseleft,\@yquant@enclosetop)%
         }%
         \expandafter\ifx\csname yquant@circuit@enclose@#1@involved\endcsname\yquant@circuit@enclose@allinvolved%
            \csgundef{yquant@circuit@enclose@#1@extentT@1:1}%
            \csgundef{yquant@circuit@enclose@#1@extentB@1:\csname yquant@env1@registers\endcsname}%
         \else%
      ...
      \or%
         ...
         \yquant@draw@enclose@calcleft{#1}\@yquant@i%
         \yquant@draw@enclose@calcright{#1}\@yquant@i%
         \edef\@yquant@points{%
            \yquant@coords(\@yquant@encloseleft,\@yquant@enclosebottom)%
            \yquant@coords(\@yquant@encloseright,\@yquant@enclosebottom)%
         }%
         % Then we go up at the right
         \yquant@fordown \@yquant@i := \numexpr\@yquant@i-1\relax downto 0 {%
            % first we calculate the next right position
            \let\@yquant@encloseoldright=\@yquant@encloseright%
            \yquant@draw@enclose@calcright{#1}\@yquant@i%
            \unless\ifdim\@yquant@encloseoldright=\@yquant@encloseright\space% only if the right position changes, a new path segment follows
               \yquant@draw@enclose@calctop{#1}{\the\numexpr\@yquant@i+1\relax}%
               \yquant@draw@enclose@calcbottom{#1}\@yquant@i%
               \dimdef\@yquant@midpoint{.5\dimexpr\@yquant@enclosetop\relax+.5\dimexpr\@yquant@enclosebottom\relax}% to prevent overflow
               \eappto\@yquant@points{%
                  \yquant@coords(\@yquant@encloseoldright,\@yquant@midpoint)%
                  \yquant@coords(\@yquant@encloseright,\@yquant@midpoint)%
               }%
            \fi%
         }%
         ...
         \yquant@dimenb=\@yquant@encloseleft\space% figure out the leftmost position
         % Let's go down at the left
         \yquant@for \@yquant@i := 1 to \numexpr\yquant@sort@count-1\relax {%
            \let\@yquant@encloseoldleft=\@yquant@encloseleft%
            \yquant@draw@enclose@calcleft{#1}\@yquant@i%
            \unless\ifdim\@yquant@encloseoldleft=\@yquant@encloseleft\space%
               \yquant@draw@enclose@calcbottom{#1}{\the\numexpr\@yquant@i-1\relax}%
               \yquant@draw@enclose@calctop{#1}\@yquant@i%
               \dimdef\@yquant@midpoint{.5\dimexpr\@yquant@enclosetop\relax+.5\dimexpr\@yquant@enclosebottom\relax}%
               \eappto\@yquant@points{%
                  \yquant@coords(\@yquant@encloseoldleft,\@yquant@midpoint)%
                  \yquant@coords(\@yquant@encloseleft,\@yquant@midpoint)%
               }%
               \ifdim\@yquant@encloseleft<\yquant@dimenb%
                  \yquant@dimenb=\@yquant@encloseleft\space%
               \fi%
            \fi%
         }%
      ...
      \xappto\yquant@draw@enclose@drawcommands{%
         \yquant@draw@enclose@handlelayers{#1}{\pgfkeysvalueof{/yquant/enclose/layer}}{\pgfkeysvalueof{/yquant/enclose/label layer}}{%
            \noexpand\node[{shape=yquant-enclose-polygon, /yquant/enclose/shape/polygon/points={\@yquant@points},
                            /yquant/enclose/every enclose,
               ...
               \yquant@coords(\the\yquant@dimenb, \the\yquant@dimena) {};
         }%
      }%
   \endgroup%
...
\protected\def\yquant@draw@enclose@align#1{%
   ...
   \begingroup%
      \yquant@register@peek@maxxlist\@yquant@newx{#1}%
      \yquant@dimena=\@yquant@newx\space%
      \let\pgfshapeclippathhorzresult=\empty%
...
\protected\def\yquant@draw@enclose@align@loop#1#2{%
   \begingroup%
      ...
      \yquant@register@set@x{#2}\@yquant@newx%
...
\protected\long\def\yquant@draw@enclose@handlelayers#1#2#3#4{%
   ...
   \yquant@begingroup%
      ...
      \ifstrequal{#2}{#3}{%
         ...
         \yquant@endgroup%
         \edef\@yquant@drawrest{%
            \noexpand\path[late options={/yquant/internal/name unaliased=yquantbox, /yquant/internal/dename, /yquant/enclose/every enclose,
                                         \unexpanded\expandafter\expandafter\expandafter{\csname yquant@circuit@enclose@#1@style\endcsname}}];%
         }%
         \ifstrequal{#3}{main}{%
            \yquant@draw@enclose@beamercommand{\@yquant@drawrest}%
         }{%
            \pgfonlayer{#3}%
               \yquant@draw@enclose@beamercommand{\@yquant@drawrest}%
            \endpgfonlayer%
