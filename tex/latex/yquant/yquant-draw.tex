...
\protected\long\def\yquant@draw@subcircuit@prepare#1#2#3#4#5{%
   ...
   \yquant@begingroup%
         ...
         \begingroup%
            ...
            \else%
               ...
               \ifyquanthorz{%
                  ...
                  \dimen0=.5\dimexpr\pgf@pt@aa\dimexpr\pgf@picminy+\pgf@picmaxy-2\pgf@pt@y\relax-\pgf@pt@ab\dimexpr\pgf@picminx+\pgf@picmaxx-2\pgf@pt@x\relax\relax%
               }{%
               % x cos alpha + y sin alpha
                  \dimen0=.5\dimexpr\pgf@pt@aa\dimexpr\pgf@picminx+\pgf@picmaxx-2\pgf@pt@x\relax+\pgf@pt@ab\dimexpr\pgf@picminy+\pgf@picmaxy-2\pgf@pt@y\relax\relax%
               }%
               \ifdim\dimen2<0pt % reflections
                  \dimen2=-\dimen2 %
               \fi%
      ...
      \dimen4=\yquant@orientation@minus\maxdimen %
      \forlistloop\yquant@draw@subcircuit@remeasure{#5}%
      \advance \dimen4 by \yquant@orientation@plus\yquant@circuit@endwires@operatorsep\space%
      \gundef\yquant@circuit@endwires@operatorsep%
      \dimen6=\@firstofthree#4 %
      \dimdef\@yquant@delta{%
         \dimen4%
         -\yquant@draw@@x%
         \yquant@orientation@minus.5\dimen6%
      }%
      \ifdim\dimexpr\@yquant@delta\yquant@orientation@minus100sp\relax\ifyquanthorz><0pt %
         ...
         \advance \dimen6 by \yquant@orientation@plus\@yquant@delta\space%
      \fi%
      \edef\@yquant@cmd{%
         \noexpand\path \yquant@coords(\yquant@draw@@x, \the\dimen0)%
            node[/yquant/every operator, \yquant@draw@@style,%
                 ...
                 name prefix=, name suffix=, name=yquantbox]%
            {\ifyquanthorz{\vbox to \the\dimen2 {\hbox to \the\dimen6 {}}}%
                          {\hbox to \the\dimen2 {\vbox to \the\dimen6 {}}}};%
      }%
      \ifyquant@config@draw@quality%
         \@yquant@cmd%
      \else%
...
\protected\def\yquant@draw@subcircuit@remeasure#1{%
   \ifdim\dimen4\ifyquanthorz<>\yquant@register@get@x{#1} %
      \dimen4=\yquant@register@get@x{#1} %
   \fi%
}
...
\protected\def\yquant@draw@stopenclose#1{%
   ...
   \begingroup%
      ...
      \dimen0=\@yquant@enclosebottom\space% store the bottom-most position
      \ifcase\csname yquant@circuit@enclose@#1@shape\endcsname\space%
         ...
         \fi%
         \dimdef\@yquant@encloseleft{\csname yquant@draw@enclose@#1@left\endcsname-\dimen4}%
         \dimen2=\@yquant@encloseleft\space%
         \dimdef\@yquant@encloseright{\csname yquant@draw@enclose@#1@right\endcsname+\dimen4}%
      ...
      \or%
         ...
         \dimen2=\@yquant@encloseleft\space% figure out the leftmost position
         ...
         \yquant@for \@yquant@i := 1 to \numexpr\yquant@sort@count-1\relax {%
            ...
            \unless\ifdim\@yquant@encloseoldleft=\@yquant@encloseleft\space%
               ...
               \ifdim\@yquant@encloseleft<\dimen2 %
                  \dimen2=\@yquant@encloseleft\space%
               \fi%
            \fi%
         }%
