...
\protected\def\yquant@draw@init#1#2{%
   ...
   \forlistloop{\yquant@draw@enclose@declare{#1}}\yquant@circuit@enclose@active% make it known
}
...
\protected\def\yquant@draw@enclose@declare#1#2{%
   % this is basically the same as measure, just for a single wire initialization, which has no width
   \begingroup%
      \ifcase\csname yquant@circuit@enclose@#2@shape\endcsname\space%
         % is any of the registers in the target list relevant for our enclose?
         \ifyquant@OR\expandafter\ifx\csname yquant@circuit@enclose@#2@involved\endcsname\yquant@circuit@enclose@allinvolved\fi%
                     \ifcsname yquant@circuit@enclose@#2@extentT@\yquant@register@resolvetocolon{#1}\endcsname\fi{%
            \edef\yquant@draw@@x{\yquant@register@get@x{#1}}%
            \ifdim\csname yquant@draw@enclose@#2@left\endcsname>\dimexpr\yquant@draw@@x\relax%
               \csdimgdef{yquant@draw@enclose@#2@left}{\yquant@draw@@x}%
            \fi%
            \ifdim\csname yquant@draw@enclose@#2@right\endcsname<\dimexpr\yquant@draw@@x\relax%
               \csdimgdef{yquant@draw@enclose@#2@right}{\yquant@draw@@x}%
            \fi%
         }\relax%
      \or%
         \edef\leftpos{\yquant@register@get@x{#1}}%
         \let\rightpos=\leftpos%
         \yquant@draw@enclose@measure@register{#2}{#1}%
      \else%
         \PackageError{yquant.sty}{Internal error}{Unknown enclose shape type (2b).}%
      \fi%
   \endgroup%
}
...
\protected\def\yquant@draw@enclose@align@loop#1#2{%
   \begingroup%
      ...
      \expandafter\yquant@circuit@extendwire\expandafter{#2}*%
   \endgroup%
}
