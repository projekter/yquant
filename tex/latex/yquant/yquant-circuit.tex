...
\protected\def\yquant@circuit@flushwire#1{%
   \ifcsname\yquant@prefix registermap@#1\endcsname%
      ...
      \ifdim\pgf@picminx<\yquant@draw@subcircuit@outermin%
         \xdef\yquant@draw@subcircuit@outermin{\the\pgf@picminx}%
      \fi%
      \global\pgf@picminx=\storedleft%
   \else%
      \yquant@draw@wire{#1}0%
...
\protected\def\yquant@circuit@operator#1#2#3{%
   ...
   % if x > 1sp, everything is fine. We already had some gates.
   \ifdim\yquant@circuit@operator@x>1sp %
      \dimdef\yquant@circuit@operator@x{%
         \yquant@circuit@operator@x+\yquant@config@operator@sep%
      }%
   \else%
      % if x = 1sp, this means we insert the operator to a register that had no previous gates, but which has an initial text.
      \ifdim\yquant@circuit@operator@x=1sp %
         % just add the separation, drop this special 1sp marker
         \let\yquant@circuit@operator@x=\yquant@config@operator@sep%
      \else%
         % there was no text and no operation before. For seamless circuits, start right at zero; for others, only the init gate will start at zero.
         \ifyquant@env@seamless\relax{%
            \unless\ifx\yquant@draw@@multi\yquant@draw@@multiinit%
               \let\yquant@circuit@operator@x=\yquant@config@operator@sep%
            \fi%
         }%
      \fi%
...
% #3: parameter(s)
% #4: 0/1 if this is to be applied also now
...
\protected\def\yquant@circuit@output{%
   \csxappto{\yquant@prefix outputs}%
            {\yquant@circuit@output@draw%
               {\unexpanded\expandafter{\yquant@lang@attr@value}}%
               {\unexpanded\expandafter{\yquant@attrs@remaining}}%
               {\unexpanded\expandafter{\yquant@register@get@ids@list}}%
               {\yquant@register@get@ids@min}{\yquant@register@get@ids@min}%
               {\yquant@register@get@ids@count}%
            }%
}

\protected\def\yquant@circuit@output@draw#1#2#3#4#5#6{%
   ...
   \let\yquant@circuit@operator@x=\yquant@env@end@xpos%
   \yquant@draw@{#1}{}%
}
% END_FOLD