...
\protected\def\yquant@circuit@operator#1#2#3{%
   ...
   \ifyquant@env@seamless{%
      \ifdim\yquant@circuit@operator@x>0pt %
         \dimdef\yquant@circuit@operator@x{%
            % we need the separation for the initial gate only if there was a label; for this we assign the dummy value of 1sp in this case. Just to be extra meticulous, we remove it here again.
            \ifdim\yquant@circuit@operator@x=1sp 0pt\else\yquant@circuit@operator@x\fi+%
...
% #1: {additional height of subcircuit box}{additional depth of subcircuit box}
% #2: style
\protected\def\yquant@circuit@subcircuit#1#2{%
   \numdef\yquant@circuit@subcircuit@id{\yquant@env+1}%
   \listcsxadd{\yquant@prefix subcircuits}{\yquant@circuit@subcircuit@id}%
   \begingroup%
      \tikzset{/yquant/every operator, #2,%
      ...
      \yquant@env@begin@noarg%
         ...
         \advance\dimen2 by \@firstoftwo#1 %
         \ifnum\firstinput>1 %
         ...
         \advance\dimen4 by \@secondoftwo#1 %
