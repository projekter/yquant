...
\protected\def\yquant@circuit@extendwire#1#2{%
   \begingroup%
      \ifx*#2%
         \edef\@yquant@wirexpos{\the\yquant@dimena}%
      \else%
         \pgfpointanchor{yquantbox}{#2}%
         \edef\@yquant@wirexpos{\the\yquant@pgf@x}%
      \fi%
      \ifdim\yquant@orientation@plus\@yquant@wirexpos>0pt % may be negative for init gates
         ...
         \ifdefined\yquant@config@corner@at%
            \ifinlist{#1}\yquant@config@corner@at{%
               \ifnum\yquant@register@types@cbit>\yquant@draw@@currentcontroltype%
                  \dimdef\@yquant@wirexpos{\@yquant@wirexpos\yquant@config@corner@xamount}%
               \else%
                  \dimdef\@yquant@wirexpos{\@yquant@wirexpos+5\dimexpr\yquant@config@corner@xamount\relax}%
               \fi%
            }\relax%
         \fi%
         \ifnum\@yquant@wiretype=\yquant@register@types@nobit%
            ...
            \yquant@register@set@lastwire{#1}{%
               {\@yquant@wirexprevpos}{\@yquant@wirexpos}{}%
   ...
   \yquant@for \yquant@circuit@endwires@i := 1 to \csname\yquant@prefix registers\endcsname {%
      ...
      \ifyquant@register@isaliased\yquant@circuit@endwires@i{%
         \xifinlistcs\yquant@circuit@endwires@i{\yquant@prefix inonly}{%
            ...
            \ifyquanthorz{%
               \edef\@yquant@storedleft{\the\pgf@picminx}%
               \yquant@draw@wire\yquant@circuit@endwires@i1%
               \global\pgf@picminx=\@yquant@storedleft%
            }{%
               \edef\@yquant@storedtop{\the\pgf@picmaxy}%
               \yquant@draw@wire\yquant@circuit@endwires@i1%
               \global\pgf@picmaxy=\@yquant@storedtop%
            }%
...
\protected\def\yquant@circuit@flushwire#1{%
   ...
   \begingroup%
      ...
      \yquant@register@set@lastwire{#1}{%
         {\expandafter\@secondoffour\@yquant@wirelast}{\expandafter\@secondoffour\@yquant@wirelast}{}%
         {\unexpanded\expandafter\expandafter\expandafter{%
...
\protected\def\yquant@circuit@extendcontrolline@corner#1{%
   \ifyquant@OR%
      ...
      \ifnum\yquant@register@types@qubit<\yquant@register@get@type{#1} %
         ...
         \begingroup%
            \edef\@yquant@wiretype{\yquant@register@get@type{#1}}%
...
\protected\def\yquant@circuit@actonwires#1#2#3#4{%
   \begingroup%
      ...
      \def\@yquant@do##1{%
         ...
         \eappto\@yquant@tmp{#2{##1}#4}%
         #1{##1}#4%
      }%
      \forlistloop\@yquant@do{#3}%
      \csxappto{\yquant@prefix draw}{\@yquant@tmp}%
   \endgroup%
}
...
\pgfkeys{%
   /yquant/enclose filter handler/.code={%
      \ifyquant\ifnum\pgfkeyscasenumber=0 \fi{%
         \@firstoftwo%
      }{%
         \expandafter\ifyquant@beginswith\expandafter{\pgfkeyscurrentkey}{/yquant/enclose}\@secondoftwo\@firstoftwo
      }%
      {%
         \eappto#1{%
         ...
         \eappto#1{%
            ,\unexpanded\expandafter{\pgfkeyscurrentkey}%
            \unless\ifx\pgfkeyscurrentvalue\pgfkeysnovalue@text%
               ={\unexpanded\expandafter{\pgfkeyscurrentvalue}}%
            \fi%
         }%
...
\def\yquant@circuit@startenclose@i[#1]#2{%
   \begingroup%
      ...
      \ifyquant\ifdefined\yquant@circuit@enclose@currentbeamercommand\fi{%
         ...
         \yquant@cleanup@csadd{yquant@circuit@enclose@#1@beamercommand}%
...
\protected\def\yquant@circuit@stopenclose@i#1{%
   ...
   {%
      \begingroup%
         ...
         \csgundef{yquant@circuit@enclose@#1@visited}% only defined for encloseall
         \csgundef{yquant@config@enclose@#1@from}%
         \csgundef{yquant@config@enclose@#1@to}%
...
\def\yquant@circuit@encloseall@start#1#2{%
   ...
   {%
      ...
      \begingroup%
         \let\yquant@circuit@enclose@style=\empty%
         % we still want to rewrite everything in /yquant/enclose to /tikz
         \pgfkeys{%
            /pgf/key filters/false/.install key filter,%
            /yquant/enclose filter handler/.install key filter handler=\yquant@circuit@enclose@style,
         }%
         \tikz@enable@node@quotes%
         \pgfqkeysfiltered{/yquant/enclose}{#2}%
         \global\cslet{yquant@circuit@enclose@#1@style}\yquant@circuit@enclose@style%
      \endgroup%
      ...
      \global\cslet{yquant@circuit@enclose@#1@visited}\empty% but we still need to keep track of the ones we visited
...
\protected\def\yquant@circuit@enclose@alignall#1{%
   \yquant@for \@yquant@i := 1 to \csname yquant@env#1@registers\endcsname {%
      \listeadd\yquant@circuit@enclose@align@list{#1:\@yquant@i}%
   }%
}
...
\protected\def\yquant@circuit@enclose@relevant@i#1#2#3#4#5{%
...
\protected\def\ifyquant@circuit@enclose@relevant#1#2{%
   % expand to #3 if a non-all enclose #1 contains the register #2 (in the current prefix). No else clause. This marks the register as active during the enclose.
   \begingroup%
      \ifcsname yquant@config@enclose@#1@from\endcsname%
         ...
         \ifcsname yquant@config@enclose@#1@fromresolve\endcsname%
            ...
            \expandafter\yquant@register@singlepeek%
               \expandafter\newid%
               \csname yquant@circuit@enclose@#1@prefixid\expandafter\expandafter\expandafter\endcsname%
               \expandafter\expandafter\expandafter{\csname yquant@config@enclose@#1@fromresolve\endcsname}0%
            \ifdefined\newid%
               \listcsxadd{yquant@config@enclose@#1@from}{%
                  \expandafter\yquant@register@fullresolvetocolon%
                     \csname yquant@circuit@enclose@#1@prefixid\endcsname%
                     \newid%
               }%
               \csgundef{yquant@config@enclose@#1@fromresolve}%
         ...
         \expandafter\expandafter\expandafter\forlistcsloop\expandafter\expandafter\expandafter{%
            \expandafter\expandafter\expandafter\yquant@register@colonsplit\expandafter\expandafter\expandafter{%
               \expandafter\expandafter\expandafter\yquant@circuit@enclose@relevant@i%
               \expandafter\expandafter\expandafter{\yquant@env@prefix@id\yquant@prefix}%
               {#2}
               <%
            }%
         }{yquant@config@enclose@#1@from}%
      \else%
         \let\@yquant@next=\@firstofone%
      \fi%
      \@yquant@next{%
         \ifcsname yquant@config@enclose@#1@to\endcsname%
            ...
            \ifcsname yquant@config@enclose@#1@toresolve\endcsname%
               \expandafter\yquant@register@singlepeek%
                  \expandafter\newid%
                  \csname yquant@circuit@enclose@#1@prefixid\expandafter\expandafter\expandafter\endcsname%
                  \expandafter\expandafter\expandafter{\csname yquant@config@enclose@#1@toresolve\endcsname}3{%
                  % it was a vector register
                  \ifdefined\newid%
                     \edef\@yquant@do{%
                        \noexpand\yquant@register@colonsplit{\yquant@circuit@enclose@relevant@i>{#2}}%
                           \expandafter\yquant@register@fullresolvetocolon%
                              \csname yquant@circuit@enclose@#1@prefixid\endcsname%
                              \newid%
                     }%
                     \expandafter\@yquant@do\expandafter\@gobble\yquant@list@endchar% we need to eat up the \listbreak, if it comes
                     \global\cslet{yquant@config@enclose@#1@toresolved}\relax%
                  \else%
                  ...
                  \ifdefined\newid%
                     ...
                     \listcsxadd{yquant@config@enclose@#1@to}{%
                        \expandafter\yquant@register@fullresolvetocolon%
                           \csname yquant@circuit@enclose@#1@prefixid\endcsname%
                           \newid%
                     }%
...
\protected\def\yquant@circuit@enclose@update@extentTB@i#1#2#3#4{%
   ...
   \begingroup%
      \ifyquant\expandafter\ifx\csname yquant@circuit@enclose@#4@involved\endcsname\yquant@circuit@enclose@allinvolved\fi{%
         \edef\@yquant@curcolon{\yquant@register@resolvetocolon{#2}}%
         % if we visit a register for the first time and it is not empty, then we cannot move init labels to the front
         \xifinlistcs\@yquant@curcolon{yquant@circuit@enclose@#4@visited}\relax{%
            \listcsxadd{yquant@circuit@enclose@#4@visited}\@yquant@curcolon%
         ...
         \ifyquant@circuit@enclose@relevant{#4}{#2}{%
            ...
            \ifnum\csname yquant@circuit@enclose@#4@spacing\endcsname=0 %
               \xifinlistcs\@yquant@curcolon{yquant@circuit@enclose@#4@involved}\relax{%
                  \listcsxadd{yquant@circuit@enclose@#4@involved}\@yquant@curcolon%
                  \yquant@circuit@enclose@allowmovelabel{%
            ...
            \else%
               \ifcsname yquant@circuit@enclose@#4@extentT@\@yquant@curcolon\endcsname%
                  \ifdim\csname yquant@circuit@enclose@#4@extent#1@\@yquant@curcolon\endcsname<#3 %
                     \csxdef{yquant@circuit@enclose@#4@extent#1@\@yquant@curcolon}{#3}%
                  \fi%
               \else%
                  \csxdef{yquant@circuit@enclose@#4@extent#1@\@yquant@curcolon}{#3}%
                  \csxdef{yquant@circuit@enclose@#4@extent\ifx T#1B\else T\fi @\@yquant@curcolon}{0pt}%
                  \listcsxadd{yquant@circuit@enclose@#4@involved}{\@yquant@curcolon}%
                  \yquant@circuit@enclose@allowmovelabel{%
                     \global\cslet{yquant@circuit@enclose@#4@allowmovelabel}\@secondoftwo%
                  }%
...
\expandafter\protected\expandafter\def\csname yquant@circuit@enclose@storeextents@2\endcsname#1#2{%
   ...
   \edef\@yquant@tmp{\yquant@register@peek@height{#1}{#2}}%
   \ifdim\@yquant@tmp>\csname yquant@circuit@enclose@\yquant@circuit@enclose@id @extentT@#1:#2\endcsname\space%
      \global\cslet{yquant@circuit@enclose@\yquant@circuit@enclose@id @extentT@#1:#2}\@yquant@tmp%
   \fi%
   \edef\@yquant@tmp{\yquant@register@peek@depth{#1}{#2}}%
   \ifdim\@yquant@tmp>\csname yquant@circuit@enclose@\yquant@circuit@enclose@id @extentB@#1:#2\endcsname\space%
      \global\cslet{yquant@circuit@enclose@\yquant@circuit@enclose@id @extentB@#1:#2}\@yquant@tmp%
   \fi%
}
% END_FOLD