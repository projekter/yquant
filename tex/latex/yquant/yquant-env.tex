...
\def\yquant@env@prefix@id{\romannumeral-`0\expandafter\yquant@env@prefix@id@\romannumeral-`0}
\def\yquant@env@prefix@id@ yquant@env#1@{#1}

% sync with yquantlanguage-groups
\let\yquant@env@hierarchy=\empty%
\long\protected\def\yquant@env@begin@generic#1[#2]{%
   \begingroup%
      ...
      \listeadd\yquant@env@hierarchy{\the\yquant@env}%
      \ifnum\yquant@env=1 %
         \yquantmeasuringtrue%
         \yquant@env@substitute%
         \global\cslet{\yquant@prefix parameters}\empty%
         \global\let\yquant@draw@enclose@drawcommands\empty%
      \else%
...
\protected\def\yquant@env@end{%
         \ifnum\csname\yquant@prefix registers\endcsname>0 %
            \yquant@begingroup%
               ...
               \yquant@for \@yquant@i := 1 to \csname\yquant@prefix registers\endcsname {%
                  \yquant@register@execclear@lastgate{\@yquant@i}{@end}%
               }%
            \yquant@endgroup%
            ...
            \unless\ifdefined\yquant@parent%
               \yquant@circuit@enclose@storeextents2%
               \yquant@env@end@calcypositions%
               \csxappto{\yquant@prefix draw}{%
                  \ifyquantdebug%
                     \show\noexpand\yquant@draw@enclose@drawcommands%
                  \fi%
                  % this has to come before the endscope, so that we still know about our layout
                  \noexpand\yquant@draw@enclose@drawcommands%
               }%
            \fi%
         \else%
      ...
      \ifdefined\yquant@parent%
         ...
         \fi%
      \else%
         \unless\ifx\yquant@circuit@enclose@active\empty%
            \PackageError{yquant.sty}{Unbalanced encloses}%
                         {Every \string\startenclose\space has to be paired with a \string\stopenclose. There are still unclosed enclosing regions.}%
         \fi%
         ...
         \gundef\yquant@draw@enclose@drawcommands%
         \global\yquant@circuit@enclose@count=0 %
         \global\yquant@env=0 %
      \fi%
   \endgroup%
}
...
\protected\def\yquant@env@end@calcypositions{%
   \begingroup%
      ...
      % Before we forget about all the height/depth data, we need to store it for our enclose regions
      \yquant@circuit@enclose@storeextents0%
...
\protected\def\yquant@env@end@setypositions#1{%
   ...
   \else%
      \yquant@for \i := 1 to \csname\yquant@prefix registers\endcsname {%
         \ifyquant@register@isaliased\i\relax{%
            \yquant@register@set@y\i{\ifyquanthorz{-}{}\csname\yquant@prefix env@endy@\i\endcsname}%
         }%
      }%
   \fi%
...
\listadd\yquant@env@substitutions\yquant@env@enclose
\protected\def\yquant@env@enclose{%
   \let\enclose=\yquant@circuit@enclose%
   \let\encloseall=\yquant@circuit@encloseall%
   \let\startenclose=\yquant@circuit@startenclose%
   \let\stopenclose=\yquant@circuit@stopenclose%
}
   ...
   \protected\def\yquant@env@substbeamer{%
      ...
      \mode<presentation>{
         ...
         \newcommand<>\yquant@env@beamer@onslidenoargs[2]{%
            ...
            \yquant@for \@yquant@i := 1 to \csname\yquant@prefix registers\endcsname{%
               \eappto\yquant@env@beamer@onslideappdraw{\yquant@circuit@flushwire{\@yquant@i}}%
            }%
            \csxappto{\yquant@prefix draw}%
...
\protected\def\yquant@env@check@{%
   ...
   \ifx\yquant@env@nextchar[%
      \let\@yquant@next=\yquant@langhelper@check@attrs%
   \else%
      \ifcat\noexpand\yquant@env@nextchar A% letter
         \let\@yquant@next=\yquant@langhelper@check@name%
      \else%
         \ifcat\noexpand\yquant@env@nextchar\bgroup%
            \endgroup%
            \let\@yquant@next=\yquant@env@opengroup%
         \else%
            \ifcat\noexpand\yquant@env@nextchar\egroup%
               \endgroup%
               \let\@yquant@next=\yquant@env@closegroup%
            \else%
               \ifx\yquant@env@nextchar\par%
                  \let\@yquant@next=\yquant@env@gobblepar%
               \else%
