...
\long\protected\def\yquant@env@begin@generic#1[#2]{%
   \begingroup%
      ...
      \ifnum\yquant@env=1 %
         \yquantmeasuringtrue%
         \forlistloop{##1}
         \yquant@env@substitute#1\yquant@env@contscan%
...
\def\yquant@env@substitute#1#2{%
   \forlistloop{\yquant@env@substitute@{#1}{#2}}\yquant@env@substitutions%
}

\def\yquant@env@substitute@#1#2#3{%
   #3{#1}{#2}%
}
...
\protected\def\yquant@env@substikz#1#2{%
...
\ifdefined\beamer@masterdecode%
   \def\yquant@env@check@noendgroup{}
   \protected\def\yquant@env@check@endgroupnonbeamer{%
      ...
      \ifyquant@OR{\ifx\yquant@env@nextchar\yquant@env@beamer@only}%
                  {\ifx\yquant@env@nextchar\yquant@env@beamer@alt}\relax{%
         \ifyquant@OR{\ifx\yquant@env@nextchar\yquant@env@beamer@temporal}%
                     {\ifx\yquant@env@nextchar\yquant@env@check@noendgroup}\relax{%
            \endgroup%
         }%
      }%
   }%
   ...
   \protected\def\yquant@env@substbeamer#1#2{%
      % standard case:
      % #1 = \yquant, #2 = \yquant@env@contscan (i.e., without grouping)
      \let\yquant@env@beamer@store@only=\only%
      \let\yquant@env@beamer@store@alt=\alt%
      \let\yquant@env@beamer@store@temporal=\temporal%
      \let\yquant@env@beamer@store@uncover=\uncover%
      \let\yquant@env@beamer@store@visible=\visible%
      \let\yquant@env@beamer@store@invisible=\invisible%
      %
      \long\def\only##1{%
         \yquant@env@beamer@store@alt{#2##1}{#2}%
      }%
      \long\def\yquant@env@beamer@alt##1##2{%
         \yquant@env@beamer@store@alt{#2##1}{#2##2}%
      }%
      \renewcommand<>\temporal[3]{%
         \yquant@env@beamer@store@temporal##4{#2##1}{#2##2}{#2##3}%
      }%
      % There's no need for the fakeinvisible or spacingcover macros, since we are within TikZ anyway
      \long\def\yquant@env@beamer@visibilitymacros##1##2##3##4{%
         \yquant@env@beamer@visibilitymacros@flip##1%
         \yquant@env@beamer@store@alt%
            {#1##3}%
            {%
               \begingroup%
                  \letcs\yquant@env@beamer@olddraw{\yquant@prefix draw}%
                  \csglet{\yquant@prefix draw}\empty%
                  ##2{#1##3}%
                  \csxdef{\yquant@prefix draw}{%
                     \unexpanded\expandafter{\yquant@env@beamer@olddraw}%
                     \ifx1##1%
                        \noexpand\yquant@env@beamer@store@alt##4{}%
                     \else%
                        \noexpand\yquant@env@beamer@store@only##4%
                     \fi%
                     {%
                        \unexpanded{##2}{%
                           \unexpanded\expandafter\expandafter\expandafter{\csname\yquant@prefix draw}%
                        }%
                     }%
                  }%
               \endgroup%
               #1% TODO: should only call this if the parser was still active after ##2...
           }%
           ##4%
      }
      \let\uncover=\yquant@env@beamer@uncover%
      \let\visible=\yquant@env@beamer@visible%
      \let\invisible=\yquant@env@beamer@invisible%
   }
   \def\yquant@env@beamer@visibilitymacros@flip#1#2#3#4{%
      \ifx1#1%
         \expandafter\@firstoftwo%
      \else%
         \expandafter\@secondoftwo%
...
\else%
   \let\yquant@env@check@endgroupnonbeamer=\endgroup%
\fi%
            ...
            \else%
               ...
               \else%
                  \ifcat\noexpand\yquant@env@nextchar\relax%
                     \yquant@env@check@endgroupnonbeamer%
                     \let\next=\relax%
                  \else%
