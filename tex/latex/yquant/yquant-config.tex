   ...
   \ifblank{#1}{%
      ...
      \ifblank{#2}{%
         ...
         \yquant@for \@yquant@i := #1 to #2 {%
            \listeadd\yquant@config@onlyloop@idxs\@yquant@i%
         }%
      }%
   }%
}
...
\pgfqkeys{/yquant}{%
   ...
   operators/every zz/.style=%
      {shape=yquant-circle, radius=.5mm, fill},%
% END_FOLD
% BEGIN_FOLD Enclose styles
   enclose/.cd,%
   options/.is family,%
   name/.code=%
      {\unless\ifdefined\yquant@circuit@enclose@allowname%
          \PackageError{yquant.sty}{Repeated enclose names}%
                       {A name for an enclosing region may only be specified once.}%
       \fi%
       \ifcsname yquant@circuit@enclose@names@#1\endcsname%
          \PackageError{yquant.sty}{Duplicate enclose names}%
                       {The name `#1' is already defined for an enclosing region.}%
       \fi%
       \csxdef{yquant@circuit@enclose@names@#1}{\the\yquant@circuit@enclose@count}%
       \undef\yquant@circuit@enclose@allowname%
      },
   name/.belongs to family=/yquant/enclose/options,%
   from/.code=%
      {\expandafter\yquant@register@singleget%
          \csname yquant@config@enclose@\yquant@circuit@enclose@id @from\endcsname%
          {#1}0%
       \ifcsname yquant@config@enclose@\yquant@circuit@enclose@id @from\endcsname
          \csxdef{yquant@config@enclose@\yquant@circuit@enclose@id @from}{%
             \expandafter\yquant@register@resolvetocolon%
               \csname yquant@config@enclose@\yquant@circuit@enclose@id @from\endcsname%
             \yquant@list@delim%
          }%
       \else%
          % maybe the register does not exist, but it is created within the macro... So we have to store the id
          \global\cslet{yquant@config@enclose@\yquant@circuit@enclose@id @from}\empty%
          \csxdef{yquant@config@enclose@\yquant@circuit@enclose@id @fromresolve}{#1}%
       \fi%
      },
   from/.belongs to family=/yquant/enclose/options,
   to/.code=%
      {\expandafter\yquant@register@singleget%
          \csname yquant@config@enclose@\yquant@circuit@enclose@id @to\endcsname%
          {#1}3{%
          % even if the "to" register already exists, if it was a vector register with no explicitly specified index and thus could be enlarged during the macro, we always have to store the id
          \csxdef{yquant@config@enclose@\yquant@circuit@enclose@id @toresolve}{#1}%
       }{%
          % the index was specified explicitly, so we know it for sure, if it exists
          \unless\ifcsname yquant@config@enclose@\yquant@circuit@enclose@id @to\endcsname%
             \csxdef{yquant@config@enclose@\yquant@circuit@enclose@id @toresolve}{#1}%
          \fi%
       }%
       \ifcsname yquant@config@enclose@\yquant@circuit@enclose@id @to\endcsname%
          \csxdef{yquant@config@enclose@\yquant@circuit@enclose@id @to}{%
             \expandafter\yquant@register@resolvetocolon
                \csname yquant@config@enclose@\yquant@circuit@enclose@id @to\endcsname%
             \yquant@list@delim%
          }%
       \else%
          \global\cslet{yquant@config@enclose@\yquant@circuit@enclose@id @to}\empty%
       \fi%
      },
   to/.belongs to family=/yquant/enclose/options,%
   spacing/.is choice,%
   spacing/.belongs to family=/yquant/enclose/options,%
   spacing/register/.code={\def\yquant@config@enclose@spacing{0}},%
   spacing/register/.belongs to family=/yquant/enclose/options,%
   spacing/gate/.code={\def\yquant@config@enclose@spacing{1}},%
   spacing/gate/.belongs to family=/yquant/enclose/options,%
   spacing/gate or local register/.code={\def\yquant@config@enclose@spacing{2}}%,
   spacing/gate or local register/.belongs to family=/yquant/enclose/options,%
   shape/.is choice,%
   shape/.belongs to family=/yquant/enclose/options,%
   shape/rectangle/.code={\def\yquant@config@enclose@shape{0}},%
   shape/rectangle/.belongs to family=/yquant/enclose/options,%
   shape/polygon/.code={\def\yquant@config@enclose@shape{1}},%
   shape/polygon/.belongs to family=/yquant/enclose/options,%
   every enclose/.style={draw, dashed, inner spacesep=.5*\yquant@config@register@sep, inner timesep=.5*\yquant@config@operator@sep, name=myname},%
   style/.style={/yquant/enclose/every enclose/.append style={#1}},%
   every braced enclose/.style={draw=none},
   braced enclose style/.style={/yquant/enclose/every braced enclose/.append style={#1}},%
   layer/.initial=background,%
   label layer/.initial=main,%
   shape/polygon/points/.initial,%
   overlay command/.store in=\yquant@config@enclose@beamercommand,%
   brace above/.code={\tikzset{/yquant/enclose/every braced enclose}\yquant@config@enclose@brace0{#1}},%
   brace below/.code={\tikzset{/yquant/enclose/every braced enclose}\yquant@config@enclose@brace1{#1}},%
   brace left/.forward to=/yquant/enclose/brace above,%
   brace right/.forward to=/yquant/enclose/brace below,%
   every brace label/.style={label distance=4pt},%
   every brace label above or left/.style={},%
   every brace label below or right/.style={},%
   every brace label above/.style={align=center},%
   every brace label below/.style={align=center},%
   every brace label left/.style={align=right},%
   every brace label right/.style={align=left},%
   every brace/.style={draw, /pgf/decoration/brace, decorate},%
   every brace above or left/.style={},%
   every brace below or right/.style={},%
   every brace above/.style={/pgf/decoration/raise=1pt},%
   every brace below/.style={/pgf/decoration/raise=1pt, /pgf/decoration/mirror},%
   every brace left/.style={/pgf/decoration/raise=2pt, /pgf/decoration/mirror},%
   every brace right/.style={/pgf/decoration/raise=2pt},%
   brace label style/.style={/yquant/enclose/every brace label/.append style={#1}},%
   brace label above or left style/.style={/yquant/enclose/every brace label above or left/.append style={#1}},%
   brace label below or right style/.style={/yquant/enclose/every brace label below or right/.append style={#1}},%
   brace label above style/.style={/yquant/enclose/every brace label above/.append style={#1}},%
   brace label below style/.style={/yquant/enclose/every brace label below/.append style={#1}},%
   brace label left style/.style={/yquant/enclose/every brace label left/.append style={#1}},%
   brace label right style/.style={/yquant/enclose/every brace label right/.append style={#1}},%
   brace style/.style={/yquant/enclose/every brace/.append style={#1}},%
   brace style above or left/.style={/yquant/enclose/every brace above or left/.append style={#1}},%
   brace style below or right/.style={/yquant/enclose/every brace below or right/.append style={#1}},%
   brace style above/.style={/yquant/enclose/every brace above/.append style={#1}},%
   brace style below/.style={/yquant/enclose/every brace below/.append style={#1}},%
   brace style left/.style={/yquant/enclose/every brace left/.append style={#1}},%
   brace style right/.style={/yquant/enclose/every brace right/.append style={#1}},%
   bracing style/.style={/yquant/enclose/brace style={#1},/yquant/enclose/brace label style={#1}},%
   bracing style above or left/.style={/yquant/enclose/brace above or left style={#1},/yquant/enclose/brace label above or left style={#1}},%
   bracing style below or right/.style={/yquant/enclose/brace below or right style={#1},/yquant/enclose/brace label below or right style={#1}},%
   bracing style above/.style={/yquant/enclose/brace above style={#1},/yquant/enclose/brace label above style={#1}},%
   bracing style below/.style={/yquant/enclose/brace below style={#1},/yquant/enclose/brace label below style={#1}},%
   bracing style left/.style={/yquant/enclose/brace left style={#1},/yquant/enclose/brace label left style={#1}},%
   bracing style right/.style={/yquant/enclose/brace right style={#1},/yquant/enclose/brace label right style={#1}},%
}
\def\yquant@config@enclose@brace#1#2{%
   % we copy the label parsing from TikZ
   \edef\yquant@config@enclose@brace@label@default@pos{\ifnum#1=0 \ifyquanthorz{above}{left}\else\ifyquanthorz{below}{right}\fi}%
   \begingroup%
      \ifnum\the\catcode`\:=\active\relax%
         \def\tikz@next{%
            \endgroup%
            \yquant@config@enclose@brace@parse@active{#1}{#2}%
         }%
      \else%
         \def\tikz@next{%
            \endgroup%
            \@ifnextchar[%
               {\yquant@config@enclose@brace@parse@nonactive{#1}}%
               {\yquant@config@enclose@brace@parse@nonactive{#1}[]}#2:\pgf@nil%
         }
      \fi%
      \tikz@next%
}
\begingroup%
   ...
   \gdef\yquant@config@enclose@brace@parse@active#1#2{%
      \@ifnextchar[%
         {\yquant@config@enclose@brace@parse@active@i{#1}}%
         {\yquant@config@enclose@brace@parse@active@i[]}{#1}#2:\pgf@nil%
   }
   \gdef\yquant@config@enclose@brace@parse@active@i#1[#2]#3:#4\pgf@nil{%
      \ifstrempty{#4}{%
         % no position, use default
         \yquant@config@enclose@brace@parse@active@ii{#1}[{#2}]\yquant@config@enclose@brace@label@default@pos:#3:\pgf@nil%
      }{%
         \ifstrempty{#3}{%
            \yquant@config@enclose@brace@parse@active@ii{#1}[{#2}]\yquant@config@enclose@brace@label@default@pos:#4\pgf@nil%
         }{%
            \yquant@config@enclose@brace@parse@active@ii{#1}[{#2}]#3:#4\pgf@nil%
         }%
      }%
   }
   \gdef\yquant@config@enclose@brace@parse@active@ii#1[#2]#3:#4:\pgf@nil{%
      \yquant@config@enclose@brace@do{#1}[{#2}]{#3}{#4}%
   }
\endgroup%
\def\yquant@config@enclose@brace@parse@nonactive#1[#2]#3:#4\pgf@nil{%
   \ifstrempty{#4}{%
      % no position, use default
      \yquant@config@enclose@brace@parse@nonactive@i{#1}[{#2}]\yquant@config@enclose@brace@label@default@pos:#3:\pgf@nil%
   }{%
      \ifstrempty{#3}{%
         \yquant@config@enclose@brace@parse@nonactive@i{#1}[{#2}]\yquant@config@enclose@brace@label@default@pos:#4\pgf@nil%
      }{%
         \yquant@config@enclose@brace@parse@nonactive@i{#1}[{#2}]#3:#4\pgf@nil%
      }%
   }%
}
\def\yquant@config@enclose@brace@parse@nonactive@i#1[#2]#3:#4:\pgf@nil{%
   \yquant@config@enclose@brace@do{#1}[{#2}]{#3}{#4}%
}
\def\yquant@config@enclose@brace@do#1[#2]#3#4{%
   \numdef\yquant@config@enclose@braces{\yquant@config@enclose@braces+1}%
   \begingroup%
      \edef\yquant@config@enclose@brace@style{%
         \endgroup%
         \noexpand\pgfkeysalso{%
            /yquant/internal/collect brace styles={#1}{\yquant@config@enclose@braces}, /tikz/append after command={\noexpand\pgfextra{%
            \noexpand\path[/yquant/internal/collected brace decoration \yquant@config@enclose@braces, /tikz/name prefix=, /tikz/name suffix=]%
               \ifnum#1=0 %
                  (yquantbox.north west) -- %
                  (yquantbox.\ifyquanthorz{north east}{south west})%
               \else%
                  (yquantbox.\ifyquanthorz{south west}{north east}) -- %
                  (yquantbox.south east)%
               \fi;%
         }}}%
         \noexpand\tikz@@parse@label@nonactive[{/yquant/internal/collected brace label \yquant@config@enclose@braces, #2}]#3:#4:\noexpand\pgf@nil%
      }%
      \yquant@config@enclose@brace@style%
      \csgundef{/pgfk@/yquant/internal/collected brace decoration/.@cmd}%
      \csgundef{/pgfk@/yquant/internal/collected brace label/.@cmd}%
}
\protected\def\yquant@config@enclose@collectbracestyles#1#2{%
   \csxdef{pgfk@/yquant/internal/collected brace decoration #2/.@cmd}##1\pgfeov{%
      \unexpanded\expandafter\expandafter\expandafter{\csname pgfk@/yquant/enclose/every brace/.@cmd\endcsname##1\pgfeov}%
      \unexpanded\expandafter\expandafter\expandafter{\csname pgfk@/yquant/enclose/every brace \ifnum#1=0 above or left\else below or right\fi/.@cmd\endcsname##1\pgfeov}%
      \unexpanded\expandafter\expandafter\expandafter{\csname pgfk@/yquant/enclose/every brace \ifnum#1=0 \ifyquanthorz{above}{left}\else\ifyquanthorz{below}{right}\fi/.@cmd\endcsname##1\pgfeov}%
   }%
   \csxdef{pgfk@/yquant/internal/collected brace label #2/.@cmd}##1\pgfeov{%
      \unexpanded\expandafter\expandafter\expandafter{\csname pgfk@/yquant/enclose/every brace label/.@cmd\endcsname##1\pgfeov}%
      \unexpanded\expandafter\expandafter\expandafter{\csname pgfk@/yquant/enclose/every brace label \ifnum#1=0 above or left\else below or right\fi/.@cmd\endcsname##1\pgfeov}%
      \unexpanded\expandafter\expandafter\expandafter{\csname pgfk@/yquant/enclose/every brace label \ifnum#1=0 \ifyquanthorz{above}{left}\else\ifyquanthorz{below}{right}\fi/.@cmd\endcsname##1\pgfeov}%
   }%
}
% END_FOLD
% BEGIN_FOLD Internal styles (undocument, do not use!)
\def\yquant@config@enclose@checkmovealllabels#1{%
   \csname yquant@circuit@enclose@#1@allowmovelabel\endcsname\relax{%
...
\pgfqkeys{/yquant}{%
   ...
   internal/maybe move label/.code 2 args={%
      \yquant@config@operator@position@rightaligntrue%
      \forlistloop\yquant@config@enclose@checkmovealllabels{#2}%
      \unless\ifyquant@config@operator@position@rightalign%
         \ifdim\yquant@draw@@width<#1 %
            \def\yquant@draw@@width{#1}%
         \fi%
         \pgfkeys{/yquant/every initial label/.style={}}%
      \fi%
   },%
   ...
   internal/name unaliased/.code={%
      \let\yquant@config@internal@oldppname=\tikz@pp@name%
      \let\tikz@pp@name=\@firstofone%
      \ifx\tikz@fig@name\pgfutil@empty%
         \pgfkeysalso{/tikz/name={#1}}%
      \else%
         \pgfkeysalso{/tikz/alias={#1}}%
      \fi%
      \let\tikz@pp@name=\yquant@config@internal@oldppname%
   },%
   internal/dename/.code={
      \pgfkeysalso{/tikz/name/.code=, /tikz/alias/.code=}%
   },%
   internal/collect brace styles/.code 2 args={%
      \yquant@config@enclose@collectbracestyles{#1}{#2}%
   },%
   internal/no enclose/.is if=%
      yquant@config@internal@noenclose,%
}
\ifnum\yquant@compat<2 %
   \pgfqkeys{/yquant}{%
...
\def\yquant@config@operator@minimum@width@default{5mm}
\def\yquant@config@enclose@spacing{2}
\def\yquant@config@enclose@shape{0}
\def\yquant@config@enclose@beamercommand{\uncover}
\newif\ifyquant@config@internal@multi@main
\yquant@config@internal@multi@maintrue
\newif\ifyquant@config@internal@noenclose
