...
\protected\def\yquant@prepare@create#1#2#3{%
   ...
   \else%
      \ifdefined\yquant@lang@attr@output%
         ...
         \begingroup%
            \let\yquant@prefix=\yquant@parent%
            \unless\if\yquant@register@types@nobit\yquant@register@get@type\outermap%
               \PackageError{yquant.sty}%
...
\protected\long\def\yquant@prepare#1#2{%
   ...
   \yquant@dimenb=\yquant@config@operator@minimum@width%
   ...
   \def\@yquant@do##1{%
      \ifx\yquant@lang@attr@name\empty%
         \let\@yquant@nodename=\empty%
      \else%
         \edef\@yquant@nodename{\yquant@lang@attr@name-\idx}%
      \fi%
      ...
      \expandafter\yquant@dimena\expandafter=%
         \the\dimexpr\yquant@pgf@picmaxx-\yquant@pgf@picminx\relax\space%
      \ifdim\yquant@dimena>\yquant@dimenb%
         \yquant@dimenb=\yquant@dimena%
      \fi%
      \numdef\idx{\idx+1}%
   }%
   \forlistloop\@yquant@do\yquant@circuit@operator@targets%
   % END_FOLD
...
\protected\long\def\yquant@prepare@injection#1#2{%
   ...
   \csxdef{yquant@prepare@@injection@\the\yquant@prepare@injections}{%
      {\the\yquant@dimenb}{\unexpanded\expandafter{\yquant@circuit@operator@targets}}%
   }%
...
\protected\def\yquant@prepare@injection@loop#1{%
   \ifyquant@firsttoken\yquant@register@multi{#1}{%
      \edef\@yquant@first{\@secondoffive#1}%
      \edef\@yquant@last{\@thirdoffive#1}%
      \yquant@for \@yquant@i := \@yquant@first to \@yquant@last {%
         \yquant@register@set@lastgate\@yquant@i{%
            ...
            \noexpand\ifcsname yquant@prepare@@injection@\the\yquant@prepare@injections\noexpand\endcsname%
               ...
               \yquant@prepare@injection@clean{\@yquant@first}{\@yquant@last}%
            \noexpand\fi
            \noexpand\@gobbletwo%
         }%
      }%
...
\protected\def\yquant@prepare@inject#1{%
   ...
   \yquant@dimena=\expandafter\@firstoftwo\yquant@prepare@inject@@data\space%
   \let\@tmpconti=\yquant@register@multi@contiguous%
   \protected\def\yquant@register@multi@contiguous##1##2{%
      \yquant@for \@yquant@i := ##1 to ##2 {%
         \cslet{yquant@prepare@replacecontrol@\@yquant@i}\empty%
      }%
   }%
...
\protected\def\yquant@prepare@inject@loop#1#2{%
   \ifyquant@firsttoken\yquant@register@multi{#2}{%
      ...
      \csedef{yquant@prepare@replacecontrol@\@secondoffive#2}{%
         \noexpand\ifdim\the\yquant@dimena>\yquant@dimenb%
            \yquant@dimenb=\the\yquant@dimena\space%
         \noexpand\fi%
         \yquant@sort@eadd{%
...
\protected\def\yquant@prepare@controls@loop#1#2{%
   \ifx\yquant@lang@attr@name\empty%
      \let\@yquant@nodename=\empty%
   \else%
      \edef\@yquant@nodename{\yquant@lang@attr@name-#1\idx}%
   \fi%
...
\protected\def\yquant@prepare@multiinit#1#2#3{%
   ...
   \pgfinterruptboundingbox%
      \yquant@config@operator@multitrue%
      \yquant@env@virtualize@path%
      \edef\@yquant@first{\@secondoffive#3}%
      ...
      \ifnum\@secondoffive#3=\@thirdoffive#3 %
         % this is a single register in disguise
         \yquant@register@update@height{\@yquant@first}{\the\yquant@pgf@picmaxy}%
         \yquant@register@update@depth{\@yquant@first}{\the\dimexpr-\yquant@pgf@picminy\relax}%
      \else%
   ...
   \ifdefined\yquant@parent%
      \ifinlistcs{#3}{\yquant@prefix inonly}\relax{%
         \yquant@register@set@lastgate{#3}{%
            \yquant@prepare@output@discard{\yquant@prefix}%
         }%
      }%
   \fi%
   % determine the actual dimensions by a virtual draw command
...
\protected\long\def\yquant@prepare@output@multi#1#2#3{%
   ...
   \ifdefined\yquant@parent%
      \edef\@yquant@first{\@secondoffive#3}%
      \edef\@yquant@last{\@thirdoffive#3}%
      \yquant@for \@yquant@i := \@yquant@first to \@yquant@last {%
         \xifinlistcs\@yquant@i{\yquant@prefix inonly}\relax{%
            \yquant@register@set@lastgate\@yquant@i{%
               \yquant@prepare@output@discard{\yquant@prefix}%
            }%
         }%
...
\protected\def\yquant@prepare@subcircuit@getmaxx#1{%
   \begingroup%
      \yquant@dimena=\yquant@orientation@minus16383.99999pt %
      \def\@yquant@do##1{%
         \ifcsname\yquant@prefix xpos@##1\endcsname%
            \yquant@dimenb=\csname\yquant@prefix xpos@##1\endcsname\relax%
            \ifdim\yquant@orientation@plus\yquant@dimena<\yquant@orientation@plus\yquant@dimenb%
               \yquant@dimena=\yquant@dimenb%
            \fi%
         \else%
            \PackageError{yquant.sty}{Internal inconsistency detected}%
      ...
      \forlistloop\@yquant@do{#1}%
      \expandafter%
   \endgroup%
   \expandafter\def\expandafter\@yquant@newx\expandafter{\the\yquant@dimena}%
}

\protected\def\yquant@prepare@subcircuit@move@loop#1{%
   \cslet{\yquant@prefix xpos@#1}\@yquant@newx%
}
   ...
   \begingroup%
      ...
      \else%
         \ifyquant@env@seamless{%
            \ifdim\@yquant@newx=0pt %
               \dimdef\@yquant@newx{%
                  \@yquant@newx\yquant@orientation@plus#1%
               }%
            \else%
               \dimdef\@yquant@newx{%
                  \@yquant@newx\yquant@orientation@plus\yquant@config@operator@sep\yquant@orientation@plus#1%
               }%
            \fi%
         }{%
            \dimdef\@yquant@newx{%
         ...
         \ifyquanthorz{%
            \ifdim\@yquant@newx>\pgf@picmaxx%
               \global\pgf@picmaxx=\@yquant@newx%
            \fi%
         }{%
            \ifdim\@yquant@newx<\pgf@picminy%
               \global\pgf@picminy=\@yquant@newx%
            \fi%
         }%
         \ifyquant@config@operator@position@advance%
            \edef\@yquant@next{%
               \endgroup%
               \def\noexpand\@yquant@newx{\@yquant@newx}%
...
\protected\def\yquant@prepare@subcircuit@endwires{%
   ...
   \else%
      ...
      \begingroup%
         \yquant@dimena=\yquant@orientation@minus16383.99999pt %
         \yquant@fordown \@yquant@i := \csname\yquant@prefix registers\endcsname downto 1 {%
            \ifcsname\yquant@prefix xpos@\@yquant@i\endcsname%
               \yquant@dimenb=\csname\yquant@prefix xpos@\@yquant@i\endcsname\relax%
               \ifdim\yquant@orientation@plus\yquant@dimena<\yquant@orientation@plus\yquant@dimenb%
                  \yquant@dimena=\yquant@dimenb%
               \fi%
            \else%
      ...
      \expandafter\def\expandafter\@yquant@newx\expandafter{\the\yquant@dimena}%
      \yquant@fordown \@yquant@i := \csname\yquant@prefix registers\endcsname downto 1 {%
         \cslet{\yquant@prefix xpos@\@yquant@i}\@yquant@newx%
      }%
   \fi%
...
\protected\long\def\yquant@prepare@subcircuit#1{%
   ...
   \def\@yquant@do##1{%
      ...
      \pgfinterruptboundingbox%
         ...
         \yquant@begingroup%
            ...
            \let\yquant@draw@startenclose=\@gobble%
            \let\yquant@draw@stopenclose=\@gobble%
            ...
            \global\setbox\yquant@prepare@subcircuit@box=\ifyquanthorz\vbox\hbox to 1sp{%
         ...
         \ifyquanthorz{%
            ...
            \yquant@register@inject@extents{-\pgf@picminx}{\pgf@picmaxx}%
         }%
         \edef\@yquant@cmd{%
            \noexpand\endpgfinterruptboundingbox%
            \yquant@dimena=\the\dimexpr\yquant@pgf@picmaxx-\yquant@pgf@picminx\relax\space%
   ...
   % END_FOLD
   \yquant@prepare@controls%
   \preto\yquant@attrs@remaining{/yquant/internal/no enclose}%
   ...
   \edef\yquant@prepare@subcircuit@add{%
      \yquant@sort@eadd{%
         \yquant@draw@subcircuit@single%
            ...
            {\@yquant@nodename}\ifnum\yquant@circuit@operator@numtarget=1 1\else0\fi%
...
\protected\def\yquant@prepare@finalize@ctrl#1{%
   \let\@yquant@next=\yquant@prepare@finalize@ctrl@single%
   \ifx\yquant@draw@multi#1%
      \let\@yquant@next=\yquant@prepare@finalize@ctrl@multi%
   \else%
      ...
      \else%
         ...
         \else%
            \ifx\yquant@draw@subcircuit@multi#1%
               \let\@yquant@next=\yquant@prepare@finalize@ctrl@subcircuit@multi%
            \else%
               \ifx\yquant@draw@subcircuit@single#1%
                  \let\@yquant@next=\yquant@prepare@finalize@ctrl@subcircuit@single%
               \else%
