   ...
   \begingroup%
      ...
      \def\do##1{%
         \ifcsname\yquant@prefix xpos@##1\endcsname%
            ...
            \ifdim\yquant@direction@plus\dimen0<\yquant@direction@plus\dimen2 %
               \dimen0=\dimen2 %
            \fi%
         \else%
            \PackageError{yquant.sty}{Internal inconsistency detected}%
...
\protected\long\def\yquant@prepare@subcircuit@group#1#2#3#4#5#6#7\yquant@draw@endgroup{%
   \begingroup%
      ...
      \ifyquant@config@operator@position@rightalign%
         \ifdim#1>-\yquant@pgf@picminx%
            \global\yquant@pgf@picminx=-\dimexpr#1\relax%
         \fi%
      \else%
         \ifyquant@env@seamless{%
            \ifdim\newx=0pt %
               \dimdef\newx{%
                  \newx\yquant@direction@plus#1%
               }%
            \else%
               \dimdef\newx{%
                  \newx\yquant@direction@plus\yquant@config@operator@sep\yquant@direction@plus#1%
               }%
            \fi%
         }{%
            \dimdef\newx{%
