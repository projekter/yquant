...
% #3: register type
% #4: x position
\protected\def\yquant@prepare@create#1#2#3#4{%
   \ifdefined\yquant@lang@attr@input%
      ...
      \fi%
      \yquant@register@alias{#3}{#4}{#1}{#2}\outermap%
   \else%
         ...
         \edef\cmd{%
            \noexpand\path%
               ...
               node[/yquant/every label, /yquant/every initial label,%
                    /yquant/every \style\space label%
   ...
   \csxappto{\yquant@prefix draw}{%
      \yquant@draw@create{\csname\yquant@prefix registers\endcsname}%
                         {#4}%
                         {\style}%
   ...
   % normally, we don't have to add a separation, this is done in the next step. But for the init gate to correctly decide whether it can put text to the left, we need to be able to discriminate the two situations
   \ifdim#4=0pt %
      \unless\ifx\empty\trimmedvalue%
