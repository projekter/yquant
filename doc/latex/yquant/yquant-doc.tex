...
\usepackage{braket,hyphenat,microtype,hologo,minted2,import,ragged2e,booktabs,bookmark}
\usetikzlibrary{quotes,fit,shapes.symbols,backgrounds,quantikz2,patterns}
...
\fi
\newenvironment{option*}[3][]{\ifhmode\par\fi\addvspace{5mm}%
...
\begin{document}
   ...
   \section{Introduction}
      ...
      This beta version~0.9.1 \emph{should} be stable and interfaces are not very likely to change in an incompatible way in the future.
...
\PYG{l+s+s2}{\PYGZdq{}+\PYGZdq{}}
      ...
      \subsection[Highlighting parts of a circuit]{\compat{New in 0.9}Highlighting parts of a circuit}\label{sec:enclose}
         Sometimes, there are certain parts of a circuit that deserve special attention.
         This is often accomplished by framing them in a box or putting some kind of background on them, then adding some text at the boundary of this region.
         While this can be accomplished in \Yquant{} using various options, starting from version~0.9, the enclose functionality aims to do this most conveniently.
         Note that this is not part of the \Yquant{} language specification itself, but uses \TeX{} macros instead, for a very simple reason: Sometimes, you might want to highlight only a certain part of a gate, while other parts should stick out of it.
         This means that you cannot isolate the part that is to be highlighted as you would do it with subcircuits; instead, something with a purely visual meaning, but no intrinsic part of the circuit, is required.

         There are several ways to access the enclose functionality.
         \subsubsection[\texorpdfstring{\tex{\encloseall}}{\textbackslash encloseall}]{\tex!\encloseall[<options>]{<content>}!}\linkdef\encloseall
            This should be the choice whenever you want to highlight the gates \texttt{<content>}, while the region should span from the very first to the very last register in the circuit.
            It is basically a restricted version of the \texlink{\enclose} macro, but at the same time, it is much faster to process, as \TeX{} doesn't have to figure out which registers are involved.

            The \texttt{<options>} allow you to specify any \TikZ{} option which is attached to the enclosing rectangle.
            None of the advanced options mentioned for \texlink{\enclose} are allowed here---the configuration for \tex!\encloseall! always corresponds to \ttlink[/yquant/enclose/spacing/register]{spacing=register}, \ttlink[/yquant/enclose/shape/rectangle]{shape=rectangle}, so see the details for the configuration options on how this manifests.
            However, the bracing options may very well be used (and indeed, they are probably best used with \tex!\encloseall!).

         \subsubsection[\texorpdfstring{\tex{\enclose}}{\textbackslash enclose}]{\tex!\enclose[<options>]{<content>}!}\linkdef\enclose
            This should be used whenever you need more flexibility and the enclosing region is not from the first to the last register in the whole circuit.

            By default, the region will span from the first to the last register that is mentioned within \texttt{<content>}.
            However, you can overwrite this by using the option keys \linkdef{/yquant/enclose/from}\texttt{from=<register>} and \linkdef{/yquant/enclose/to}\texttt{to=<register>}.
            If you specify a vector register, this will correspond to the first index for \texttt{from} and to the last for \texttt{to}.
            Note that it is allowed to specify a register which does not exist yet---however, it has to be created at some point during the execution of \texttt{<content>}.
            Further note that the register has to be one in the same \emph{circuit} in which \tex!\enclose! is placed---it cannot be in a subcircuit.
            However, you can achieve this functionality in a different way using named enclosures.

            \begin{option*}{yquant/enclose}{name}!!
               You may use the \texttt{name=<name>} option to give the enclosing region a name (this is not a \TikZ{} node name, it is only valid in the context of enclosing options).
               Then, when creating the register in a subcircuit, you may specify the attribute \linkdef{enclose/set from}\texttt{enclose/set from=<name>} or \linkdef{enclose/set to}\texttt{enclose/set to=<name>} to make clear that the region \texttt{<name>} should start at this register.
            \end{option*}

            It is possible to let an \tex!\enclose! start from multiple different registers---just think of various subcircuits with ancillas that cannot be put in a particular ordering with respect to each other.
            Specify the attribute for all of the registers that should be included and \Yquant{} will make sure that the region covers all of them.

            \clearpage
            \begin{example}[Using \tex!\enclose! with explicit range specifications]
               \begin{codeexample*}
\begin{tikzpicture}
   \begin{yquant}
      qubit q[4];
      x -;
      \enclose[name=region, to={q[2]}, shade] {
         h q[2];
         subcircuit {
            [ancilla, enclose/set from=region] qubit anc;
            qubit io[2];
            [ancilla, enclose/set to=region] qubit anc[+2];
            [ancilla] qubit anc[+1];
            zz (-);
         } (q[2, 3]);
         cnot q[0] | q[3];
...
\end{tikzpicture}
               \end{codeexample*}
               The example shows nicely how, semantically, \texttt{set from} is no different than specifying \texttt{from}: as soon as one ``from'' (or ``to'') specification is present, the start or end of the range is no longer calculated automatically based on the registers addressed in the region (for then, \texttt{q[0]} from the \ttlink{cnot} would have to be in the region), but defined as the least possible value that still encloses everything that was explicitly requested.
            \end{example}

            \begin{option*}{yquant/enclose}{shape}!rectangle!
               Defines how the region that encloses certain gates looks like.

               \begin{itemize}
                  \item \linkdef{/yquant/enclose/shape/rectangle}\texttt{rectangle} \\
                     The enclosing region will by default be a rectangle from the top-/leftmost to the bottom/-rightmost gate.
                     Note that this will implicitly put an \ttlink{align} gate on all the registers that are involved before and after the enclosing---else, it would be possible to have a gate that is not part of the region, but still ends up visually in it.
                     Just the act of creating a register (without a text) will never by itself be part of an enclosing region.
                     If the register creation with value or the first initialization is within an enclose block, the block will also cover the label.
                     Note that the alignment due to the rectangular shape may prevent an initialization from extending to the left of the circuit if some other registers already contain gates.

                  \item \linkdef{/yquant/enclose/shape/polygon}\texttt{polygon} (default) \\
                     \Yquant{} will draw a polygon that tries to tightly cover all the gates within the region.
                     No alignment is performed.
                     \begin{example}[Using \tex!\enclose! with polygon shape]
                        \begin{codeexample}
\begin{tikzpicture}
   \begin{yquant}
      qubit q[5];
      ...
      x q[0, 1, 4];
      x q[0];
      \enclose[shape=polygon, fill=yellow]{
         h q[2, 3];
         h q[1-];
         h q[0-2];
         h q[1];
...
\end{tikzpicture}
                        \end{codeexample}
                        Consider the output of the same circuit, using the rectangular shape:
                        \begin{center}
                           \begin{tikzpicture}
                              \begin{yquant}
                                 qubit q[5];
            ...
            A final question is how large the enclosing region will be in space\hyp like direction (i.e., between the registers).

            \begin{option*}{yquant/enclose}{spacing}!gate or local register!
               Defines how the extents of the enclosed registers are calculated.
               Possible values:
               \begin{itemize}
                  \item \linkdef{/yquant/enclose/spacing/gate}\texttt{gate} \\
                     The enclosing region will be as large as all the gates within this region.
                     Note that you \emph{could} run into trouble with multi\hyp register gates using this option.
                     \begin{example}[Using \tex!\enclose! with \texttt{gate} spacing]
                        \begin{codeexample*}
\begin{tikzpicture}
   \begin{yquant}[enclose/spacing=gate, enclose/style={draw=none}]
      qubit q;
      \enclose[fill=red!20!white] { slash q; }
      \enclose[fill=yellow]{ x q; }
      \enclose[fill=blue!20!white]{ not q; }
   \end{yquant}
\end{tikzpicture}
                        \end{codeexample*}
                     \end{example}
                  \item \linkdef{/yquant/enclose/spacing/register}\texttt{register} \\
                     The enclosing region will be as large as the \emph{final} register height/depth, which consists of \emph{all} gates that were ever present on the corresponding registers.
                     \begin{example}[Using \tex!\enclose! with \texttt{register} spacing]
                        \begin{codeexample*}
\begin{tikzpicture}
   \begin{yquant}[enclose/spacing=register, enclose/style={draw=none}]
      qubit q;
      \enclose[fill=red!20!white] { slash q; }
      \enclose[fill=yellow]{ x q; }
      \enclose[fill=blue!20!white]{ not q; }
...
\end{tikzpicture}
                     ...
                     \end{example}
                     \vskip-3mm
                     However, this is problematic due to the way subcircuits are handled by \Yquant.
                     If you have a subcircuit which has ancillas, during the calculation of the final positioning, \Yquant{} will accumulate their extents and add it to the closest connected register, so you might suddently end up with a very large enclosing region which you did not expect.
                     \begin{example}[Unexpected behavior of \texttt{register} spacing with subcircuits]
                        \begin{codeexample}
\begin{tikzpicture}
   \begin{yquant}[enclose/style={draw=none, fill=yellow}]
      qubit before;
      qubit q;
      qubit after;
      x before, after;
      ...
      \enclose[spacing=register]{ x q; }
   \end{yquant}
\end{tikzpicture}
                        \end{codeexample}
                     \end{example}
                  \item \linkdef{/yquant/enclose/spacing/gate or local register}\texttt{spacing=gate or local register} (default) \\
                     Takes into account the minimum of the global register extent \emph{before} subcircuit ancillas are added, but at least the extent of all gates within the region\footnote{%
                     Note that here, the ``at least'' part is almost superfluous---however, it makes sure that the \ttlink{overlay} attribute, which can exclude a gate from taking part in the register extent calculation, is disregarded for enclosings.
                  }.
                  This is not a perfect solution either, a good compromise: Adding the ancillas is the very first step in the calculation of positions; and the very last step is the distribution of multi\hyp register gates, so we could still run into trouble with multi\hyp register gates; but likely, both points of view help each other.
                  \begin{example}[Using \tex!\enclose! with default \texttt{gate or local register} spacing]
                     \begin{codeexample*}
\begin{tikzpicture}
   \begin{yquant}
      qubit before;
      ...
      \enclose[draw=none, fill=yellow]{
         box {gate} (q, after);
         [overlay] box {\vrule height 18mm} q;
      }
   \end{yquant}
\end{tikzpicture}
                     \end{codeexample*}
               ...
               Let's also quickly show the output of this example both with the \texttt{gate} (left) and \texttt{register} (right) spacing:
               \begin{center}
               \begin{tikzpicture}
                  \begin{yquant}
                     qubit before;
                     ...
                     \enclose[draw=none, fill=yellow, spacing=gate]{
                        box {gate} (q, after);
                        [overlay] box {\vrule height 18mm} q;
                     }
                  \end{yquant}
               \end{tikzpicture}\qquad\begin{tikzpicture}
                  \begin{yquant}
                     ...
                     \enclose[draw=none, fill=yellow, spacing=register]{
                        box {gate} (q, after);
                        [overlay] box {\vrule height 18mm} q;
                     }
                  \end{yquant}
         ...
         \subsubsection[\texorpdfstring{\tex{\startenclose} and \tex{\stopenclose}}{\textbackslash startenclose and \textbackslash stopenclose}]{\tex!\startenclose[<options>]{<name>}! and \tex!\stopenclose{<name>}!}\linkdef\startenclose\linkdef\stopenclose
            While the \texlink{\enclose} macro is already very flexible, it requires the whole content of the enclosing region to be passed as a single \TeX{} group.
            This makes it impossible to interleave regions; but still, \Yquant{} can do it for you.
            You must now use the \tex!\startenclose! macro instead, which requires a mandatory name---given explicitly instead of via the options, but it is indeed the same thing and can therefore be used with \ttlink{enclose/set from} and \ttlink{enclose/set to}.
            The content of the enclose is then simply everything that follows until a corresponding \tex!\stopenclose! with the same name is found.
            This can be arbitrarily mixed with the group\hyp delimited \texlink{\enclose} macro.
            \begin{example}[Interleaving enclosing regions]
               \begin{codeexample}
% \usetikzlibrary{patterns}
\begin{tikzpicture}
   \begin{yquant}[enclose/style={draw=none}, enclose/shape=polygon]
      qubit q[4];
      h q[0, 2, 3];
      \startenclose[pattern=north east lines, pattern color=green!60!white]{region1}
         z q[1];
         z q[-2];
         z q[1];
      \startenclose[red, pattern=north west lines, pattern color=red]{region2}
         y q[2];
      \stopenclose{region1}
      x q[3];
      x q[3];
...
\end{tikzpicture}
            ...
            \begin{warning}[Named enclosings and custom gates]
               The naming scheme of enclosings does not respect any name mangling, therefore names are available globally and must be unique within one \ttlink{yquant} environment.
               So make sure \emph{never} to use named enclosings within \hyperref[sec:customgate]{custom gate definitions}, as their names might conflict with the ones of the circuits that use them.
               Unnamed enclosings using \texlink{\enclose} or \texlink{\encloseall} are fine, though.
            \end{warning}

            \begin{warning}[Transcending subcircuits]
               Yes, it is indeed possible to start one enclose in a subcircuit and end it after this subcircuit or vice versa.
               Note that this is much easier for the \ttlink[/yquant/enclose/shape/polygon]{polygon} shape than for rectangles!
               The rectangular enclosure has to add alignments before and after to make sure that nothing enters its region that is not supposed to be there.
               Since enclosings can transcend subcircuits, this alignment is nothing that could be reproduced just by using the \ttlink{align} gate---it requires something more sophisticated.
               The problem is that at the beginning of an enclosing, it is not even known which registers and from which subcircuits will take part.
               In fact, this is only known when the enclosing is finished, but by then, the devious user might already have exited a subcircuit in which they said \tex!\startenclose!.
               Now we are in deep trouble, because calculating the \emph{width} of the subcircuit, which is necessary for the box of the subcircuit and proper vertical alignment for example of the control line or multiple subcircuits used in the same expression (luckily, having more than one [multiregister] target is impossible here, as this would lead to a name clash), was already completed---of course without the unknown alignment.
               So what we do to get a halfway proper display in the end is that during the drawing stage, where we can rather easily perform the alignment, \Yquant{} will compare whether the precalculated width coincides with the actual one, and if not, the width is adjusted post mortem.
               Yes, indeed, for this is just an attempt not do produce too much garbage.
               The size of the box itself will be rendered correctly, but for everything that came \emph{before} or a potential control line, the size is as it was calculated without alignment.
               It would be easily possible for \Yquant{} to place the control line correctly, but gates that came before cannot be re\hyp adjusted, hence the control line could potentially overlap with them -- better not to do this.
               Let's illustrate this most monstrous failure with a tiny circuit:
               \begin{codeexample*}
\begin{tikzpicture}
   \begin{yquant}
      qubit {} a;
      qubit {} b;
      box {loooooong gate} b;
      subcircuit {
         qubit {} a;
         x a;
      \startenclose[fill=blue!20]{enc}
         y a;
      } a | b;
      z b;
      \stopenclose{enc}
...
\end{tikzpicture}
               \end{codeexample*}
               Note how here, the control line is placed where you would expect it if the \ttlink{y} gate directly followed the \ttlink{x} gate without any alignment.
               In fact, how does the alignment even work?
               First, the size of the subcircuit (without alignment) is computed.
               Naturally, the \ttlink{z} gate would come after the full length of the subcircuit, since the control on the lower register blocks the complete space.
               This is now know as the position of the \ttlink{z} gate, where we then have to align the \ttlink{y} gate at.
               The pre\hyp calculated box size for the subcircuit would be too small, so it is enlarged, and after both gates, we align once more, so that there is indeed also more space than normal before the $S$~gate.
            \end{warning}

         \subsubsection{Enclosing drawing pipeline}
            Drawing the enclosings works in multiple stages: During preparation, \Yquant{} analyzes which gates in the circuit belong to the enclosing region; it also keeps track of the individual register extents (space\hyp like, i.e., from register to register), if the shape requires this.
            During the drawing stage, \Yquant{} knows in which visual order the registers are placed in the circuit and can therefore sort them properly in order to figure out which is the topmost and which is the bottom\hyp most register.
            Furthermore, the gate extents (time\hyp like, i.e., from gate to gate) are accumulated, including all separations that are gleaned from the style in \ttlink{/yquant/enclose/every enclose} as well as the one passed explicitly to \texlink{\encloseall}, \texlink{\enclose}, or \texlink{\startenclose}.

            \begin{option*}{yquant/enclose}{every enclose}!draw, dashed, inner spacesep=.5*\pgfkeysvalueof{/yquant/register/separation}, inner timesep=.5*\pgfkeysvalueof{/yquant/operator/separation}!
               This style is applied to the enclosed region, where by default, the separations increase the region just by the half distance to their next neighboring element.
            \end{option*}

            \begin{option*}{yquant/enclose}{style}!/yquant/enclose/every enclose/.append style={#1}!
               This is a shorthand that can be used to modify the appearance of the enclosed region.
            \end{option*}

            After all the calculations are done, the necessary drawing commands for the enclosing region are \emph{stored}, but not yet executed.
            Their execution is deferred until the very end of all the drawings---this would ensure that without further measures, they would be drawn \emph{on top} of everything (having them somewhere in between would just result in a weird mix).
            This is of course not desired; therefore, the enclosing region will be drawn on a background layer, but any \texttt{label} commands will be drawn on the main layer instead, which should put them to the very front.
            However, this behavior is also configurable.

            \begin{option*}{yquant/enclose}{layer}!background!
               Defines the layer onto which the enclosing regions are drawn, excluding late options such as labels.

               Note that \Yquant{} automatically loads the \TikZ{} library \texttt{backgrounds}, which makes the \texttt{background} layer available; however, any active layer can be chosen.
               It is also possible to use \texttt{main}---note that as it is not possible to explicitly draw on the \texttt{main} layer, \Yquant{} will then simply not invoke any change of layers.
               Hence, if you put the whole \Yquant{} environment on a layer \texttt{X} different from \texttt{main}, saying \texttt{main} here will draw on \texttt{X}, \emph{not} on \texttt{main}.
            \end{option*}

            \begin{option*}{yquant/enclose}{label layer}!main!
               Defines the layer onto which all labels of enclosing regions (more precisely, everything that uses the \TikZ{} feature \texttt{append after command} or \texttt{prefix after command}) are drawn.

               The same remarks as for \ttlink{/yquant/enclose/layer} hold.
            \end{option*}

         \subsubsection{Embracing regions}
            Sometimes, you want to highlight a part of a circuit in a very unobtrusive way, simply by putting curly braces at some region to the top or bottom (resp. left/right in vertical layout) of the whole circuit.
            The \tex!\encloseall! feature already sets everything up nicely so that with a bit of \TikZ{} magic on the node that is drawn, we can easily modify the visible shape into such a curly brace.
            \Yquant{} defines styles that do exactly this.

            \begin{option*}{yquant/enclose}{brace above}!!
               Style that can be passed to an \tex!\encloseall! region which will put a curly brace above (in vertical layout: to the left of) the whole circuit.
               The style expects a label as its argument, which will subsequently be drawn above (in vertical layout: to the left of) the brace.
               Note that everything that applies to ordinary \TikZ{} labels also holds here: you may use a square bracket as the first character of the label in order to specify some styles, and you may use the colon to separate an optional label position from the label text.

               \Yquant{} will also apply the following styles to the decoration of the path:
               \begin{itemize}
                  \item \ttlink{/yquant/enclose/every brace}
                  \item \ttlink{/yquant/enclose/every brace above or left}
                  \item \ttlink{/yquant/enclose/every brace above} in horizontal layout
                  \item \ttlink{/yquant/enclose/every brace left} in vertical layout
               \end{itemize}

               \Yquant{} will apply the corresponding \texttt{every brace label} styles to the label.

               Note that only the styles that have been defined \emph{before} \texttt{brace above} is invoked are applied to the brace/label!
               This allows to apply both a brace above and below and use different styles for them.
            \end{option*}

            \begin{option*}{yquant/enclose}{brace left}!!
               This is a synonym for \ttlink{/yquant/enclose/brace above}, as this naming makes more sense for vertical circuits.
            \end{option*}

            \begin{option*}{yquant/enclose}{brace below}!!
               Style that can be passed to an \tex!\encloseall! region which will put a curly brace below (in vertical layout: to the right of) the whole circuit.
               The style expects a label as its argument, which will subsequently be drawn below (in vertical layout: to the right of) the brace.
               Note that everything that applies to ordinary \TikZ{} labels also holds here: you may use a square bracket as the first character of the label in order to specify some styles, and you may use the colon to separate an optional label position from the label text.

               \Yquant{} will also apply the following styles to the decoration of the path:
               \begin{itemize}
                  \item \ttlink{/yquant/enclose/every brace}
                  \item \ttlink{/yquant/enclose/every brace below or right}
                  \item \ttlink{/yquant/enclose/every brace below} in horizontal layout
                  \item \ttlink{/yquant/enclose/every brace right} in vertical layout
               \end{itemize}

               \Yquant{} will apply the corresponding \texttt{every brace label} styles to the label.

               Note that only the styles that have been defined \emph{before} \texttt{brace below} is invoked are applied to the brace/label!
               This allows to apply both a brace below and above and use different styles for them.
            \end{option*}

            \begin{option*}{yquant/enclose}{brace right}!!
               This is a synonym for \ttlink{/yquant/enclose/brace below}, as this naming makes more sense for vertical circuits.
            \end{option*}

            \clearpage
            \paragraph{Brace styles}\leavevmode
               \begin{option*}{yquant/enclose}{every brace}!draw, /pgf/decoration/brace, decorate!
                  This style is applied to the path drawn for every brace put to an enclosing region using any of \ttlink{/yquant/enclose/brace above}, \ttlink{/yquant/enclose/brace left}, \ttlink{/yquant/enclose/brace below}, or \ttlink{/yquant/enclose/brace right}.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace style}!/yquant/enclose/every brace/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the brace path for enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{every brace above or left}!!
                  This style is applied to the path drawn for every brace put to an enclosing region using \ttlink{/yquant/enclose/brace above} or \ttlink{/yquant/enclose/brace left} styles.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace above or left style}!/yquant/enclose/every brace above or left/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the brace path above or to the left of enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{every brace below or right}!!
                  This style is applied to the path drawn for every brace put to an enclosing region using \ttlink{/yquant/enclose/brace below} or \ttlink{/yquant/enclose/brace right} styles.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace below or right style}!/yquant/enclose/every brace below or right/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the brace path below or to the right of enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{every brace above}!/pgf/decoration/raise=1pt!
                  This style is applied to the path drawn for every brace put to an enclosing region using \ttlink{/yquant/enclose/brace above} or \ttlink{/yquant/enclose/brace left} styles---it will only be active in horizontal layout.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace above style}!/yquant/enclose/every brace above/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the brace path above enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{every brace left}!/pgf/decoration/raise=2pt, /pgf/decoration/mirror!
                  This style is applied to the path drawn for every brace put to an enclosing region using \ttlink{/yquant/enclose/brace above} or \ttlink{/yquant/enclose/brace left} styles---it will only be active in vertical layout.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace left style}!/yquant/enclose/every brace left/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the brace path to the left of enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{every brace below}!/pgf/decoration/raise=1pt, /pgf/decoration/mirror!
                  This style is applied to the path drawn for every brace put to an enclosing region using \ttlink{/yquant/enclose/brace below} or \ttlink{/yquant/enclose/brace right} styles---it will only be active in horizontal layout.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace below style}!/yquant/enclose/every brace below/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the brace path below enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{every brace right}!/pgf/decoration/raise=2pt!
                  This style is applied to the path drawn for every brace put to an enclosing region using \ttlink{/yquant/enclose/brace below} or \ttlink{/yquant/enclose/brace right} styles---it will only be active in vertical layout.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace right style}!/yquant/enclose/every brace right/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the brace path to the right of enclosing regions.
               \end{option*}

            \paragraph{Brace label styles}\leavevmode
               \begin{option*}{yquant/enclose}{every brace label}!label distance=4pt!
                  This style is applied to the label for every brace put to an enclosing region using any of \ttlink{/yquant/enclose/brace above}, \ttlink{/yquant/enclose/brace left}, \ttlink{/yquant/enclose/brace below}, or \ttlink{/yquant/enclose/brace right}.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace label style}!/yquant/enclose/every brace label/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the labels of braces in enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{every brace label above or left}!!
                  This style is applied to the label for every brace put to an enclosing region using \ttlink{/yquant/enclose/brace above} or \ttlink{/yquant/enclose/brace left} styles.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace label above or left style}!/yquant/enclose/every brace label above or left/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the labels of braces above or to the left of enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{every brace label below or right}!!
                  This style is applied to the label for every brace put to an enclosing region using \ttlink{/yquant/enclose/brace below} or \ttlink{/yquant/enclose/brace right} styles.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace label below or right style}!/yquant/enclose/every brace label below or right/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the labels of braces below or to the right of enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{every brace label above}!align=center!
                  This style is applied to the label for every brace put to an enclosing region using \ttlink{/yquant/enclose/brace above} or \ttlink{/yquant/enclose/brace left} styles---it will only be active in horizontal layout.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace label above style}!/yquant/enclose/every brace label above/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the labels of braces above enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{every brace label left}!align=right!
                  This style is applied to the label for every brace put to an enclosing region using \ttlink{/yquant/enclose/brace above} or \ttlink{/yquant/enclose/brace left} styles---it will only be active in vertical layout.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace label left style}!/yquant/enclose/every brace label left/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the labels of braces to the left of enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{every brace label below}!align=center!
                  This style is applied to the label for every brace put to an enclosing region using \ttlink{/yquant/enclose/brace below} or \ttlink{/yquant/enclose/brace right} styles---it will only be active in horizontal layout.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace label below style}!/yquant/enclose/every brace label below/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the labels of braces below enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{every brace label right}!align=left!
                  This style is applied to the label for every brace put to an enclosing region using \ttlink{/yquant/enclose/brace below} or \ttlink{/yquant/enclose/brace right} styles---it will only be active in vertical layout.
                  Note that the option has to be specified \emph{before} the brace style is invoked.
               \end{option*}

               \begin{option*}{yquant/enclose}{brace label right style}!/yquant/enclose/every brace label right/.append style={#1}!
                  This is a shorthand that can be used to modify the appearance of the labels of braces to the right of enclosing regions.
               \end{option*}

            \clearpage
            \paragraph{Combined shorthands}\leavevmode
               \begin{option*}{yquant/enclose}{bracing style}!/yquant/enclose/brace style={#1},/yquant/enclose/brace label style={#1}!
                  This is a shorthand that can be used to modify the appearance of both the brace path as well as its labels in enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{bracing style above or left}!/yquant/enclose/brace above or left style={#1},/yquant/enclose/brace above or left label style={#1}!
                  This is a shorthand that can be used to modify the appearance of both the brace path as well as its labels above or to the left of enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{bracing style below or right}!/yquant/enclose/brace below or right style={#1},/yquant/enclose/brace below or right label style={#1}!
                  This is a shorthand that can be used to modify the appearance of both the brace path as well as its labels below or to the right of enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{bracing style above}!/yquant/enclose/brace above style={#1},/yquant/enclose/brace above label style={#1}!
                  This is a shorthand that can be used to modify the appearance of both the brace path as well as its labels above enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{bracing style left}!/yquant/enclose/brace left style={#1},/yquant/enclose/brace left label style={#1}!
                  This is a shorthand that can be used to modify the appearance of both the brace path as well as its labels to the left of enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{bracing style below}!/yquant/enclose/brace below style={#1},/yquant/enclose/brace below label style={#1}!
                  This is a shorthand that can be used to modify the appearance of both the brace path as well as its labels below enclosing regions.
               \end{option*}

               \begin{option*}{yquant/enclose}{bracing style right}!/yquant/enclose/brace right style={#1},/yquant/enclose/brace right label style={#1}!
                  This is a shorthand that can be used to modify the appearance of both the brace path as well as its labels to the right of enclosing regions.
               \end{option*}
...
\end{tikzpicture}
      ...
      \clearpage
      \subsection{Defining own gates}\label{sec:customgate}
         \begin{warning}[Scope]
   ...
   \section{Reference: Gates and operations}\label{sec:gates}
      ...
      \subsection{\texorpdfstring{\yquant{qubit}}{qubit}}\linkdef{qubit}
         ...
         \begin{itemize}
            ...
            \item \yquant![enclose/set from=<enclose name>]! \\
               Tells \Yquant{} that the named enclosing \texttt{<name>} should at least start from this register (if multiple registers are created, from the first one), if not at a preceding one. \\
               See the documentation for \ttlink{/yquant/enclose/from}.
            \item \yquant![enclose/set to=<enclose name>]! \\
               Tells \Yquant{} that the named enclosing \texttt{<name>} should at least end at this register (if multiple registers are created to the last one), if not at a succeeding one. \\
               See the documentation for \ttlink{/yquant/enclose/to}.
         \end{itemize}
...
\end{tikzpicture}
            ...
            \begin{example}<Updated in 0.9, 0.4>
               \begin{codeexample*}
...
\end{tikzpicture}
               ...
               This appears to be logically preferrable to using the \texlink{\enclose} macro, as the syndrome measurement is a self-contained operation which introduces its own registers; in principle, it could be turned into a custom gate and reused.
               ...
               Finally, we will give a similar circuit by using the \texlink{\enclose} interface instead of subcircuits, this time also showing how we can change the shape of the measurement gate to one as in the \pkg{qcircuit} manual:
...
\end{tikzpicture}
            ...
            \paragraph{C. Grouping}\leavevmode
               \begin{example}<Updated in 0.9, 0.4, 0.1.1>
                  \begin{codeexample*}
\begin{tikzpicture}
...
\end{tikzpicture}
                  ...
                  Here, we use the \texlink{\enclose} macro to specify a part of the circuit that should be highlighted.
                  The time\hyp like extent is given by the earliest and latest gates in the circuit; however, note that the last register is \emph{included} in the enclosed section, but explicitly \emph{excluded} using the \ttlink[/yquant/enclose/to]{to} key, so that now both the \ttlink z\hyp gate as well as the control of the \ttlink[h]{Hadamard} are not part of the enclosed region.
...
\end{tikzpicture}
               ...
               \begin{example}<Updated in 0.9, 0.6, 0.4>
                  \begin{codeexample*}
...
\end{tikzpicture}
                  ...
                  Alternatively, we could also use the \texlink{\enclose} macro to cover the \ttlink{text}; however, this would then be drawn \emph{under} the wires (which we could fix by setting \ttlink{/yquant/enclose/layer} to \texttt{main}).
               \end{example}

            \paragraph{C. Boxing/Highlighting Parts of a Circuit}\leavevmode
               \begin{example}<Updated in 0.9, 0.2>
                  \begin{codeexample*}
% \usetikzlibrary{quotes}
\begin{tikzpicture}
   \begin{yquant*}
      h a;
      cnot b | a;
      \enclose["reversed c-\textsc{not}", solid, inner ysep=3pt] {
         h -;
         cnot b | a;
         h -;
      }
...
\end{tikzpicture}
                  \end{codeexample*}
                  Here, we could use subcircuits instead of \texlink{\enclose}, which would double the separation:
...
\begin{tikzpicture}
   \begin{yquant*}
      h a;
      \encloseall[rounded corners, fill=blue!20, "\textsc{swap}" below] {
         cnot b | a;
         cnot a | b;
         cnot b | a;
      }
...
\end{tikzpicture}
                  \end{codeexample*}
                  Since version~0.9, the \texlink{\encloseall} macro is available to comfortably wrap things in boxes.
                  The default style already draws a dashed box, so here we simply add some complements.
                  Note that by using \texlink{\encloseall}! instead of \texlink{\enclose}, we make sure that the vertical extent of the box not only encloses all the gates that are in between, but in fact covers the whole height and depth of the involved registers (which here are larger due to the size of the \ttlink[h]{Hadamard} gates).
                  Further note that the \emph{creation} of registers (without putting an initialization label) is not part of the region: in principle, the register \texttt{b} is lazily created within the \texlink{\encloseall}, but without a label, this is probably not intended to be taken into account, hence \Yquant{} decides to ignore this fact.
               \end{example}
...
\end{tikzpicture}
            ...
            \begingroup%
               ...
               \begin{example}<Updated in 0.9, 0.7>
                  \begin{codeexample*}
\long\def\explain#1#2#3{%
   \encloseall[brace left={#1}, brace right={$\displaystyle#2$}] { #3 }
}
\begin{tikzpicture}
...
\end{tikzpicture}
                  ...
                  In order to output the explanations, we basically just use the \texlink{\encloseall} macro with the \ttlink{/yquant/enclose/brace left} and \ttlink{/yquant/enclose/brace right} options, and since we need it so often, we define a macro shorthand for this.
                  However, here, the explanations should be drawn without and kind of brace---so we simply empty the \ttlink{/yquant/enclose/every brace}, effectively removing the brace completely.
...
\begin{tikzpicture}
   \begin{yquant}[vertical]
      ...
      \encloseall[brace right=Alice and Bob form an\\entangled pair of qubits] {
         h q[1];
         cnot q[2] | q[1];
      }
      setstyle {red} q[1];
      ...
      \encloseall[bracing style=red, brace left=Alice entangles\\and measures] {
         [style=red, operator/separation=0pt] cnot q[1] | q[0];
         [red] h q[0];
         [red] measure q[0, 1];
      }

      \encloseall[bracing style=blue, brace right=Bob applies gates based on\\Alice's measurements] {
...
\end{tikzpicture}
                  ...
                  Since version~0.9, \Yquant{} provides the \texlink{\encloseall} macro, which is used for highlighting a certain part of the circuit.
                  The \ttlink{/yquant/enclose/brace left} and \ttlink{/yquant/enclose/brace right} styles modify the enclosing action to cover the full register range and put the description at the outside.
               \end{example}
...
\begin{tikzpicture}
   \begin{yquant}[operators/every box/.append style={shape=yquant-circle, radius=1.5ex}]
      qubit a;
      \startenclose[brace above=above]{first}
      box {$1$} a;
      \startenclose[bracing style=blue, brace below=below]{second}
      box {$2$} a;
      \stopenclose{first}
      box {$3$} a;
      \stopenclose{second}
...
\end{tikzpicture}
               \end{codeexample*}
               This shows a complicated use of the \texlink{\enclose}\hyp like macros: here, we have overlapping enclosing regions.
               Therefore, it is not possible to use the \texlink{\enclose} macro for both descriptions (though for one would be fine).
               Instead, we create named enclosures, which can be interleaved arbitrarily.
            \end{example}

            \begin{example}<Updated in 0.9>
               \begin{codeexample*}
\begin{tikzpicture}
   \begin{yquant}
      qubit a;
      qubit b;
      qubit c;
      \enclose[draw=none, fill=red!40!white] {
         cnot b | a;
         cnot a | b;
      }
      hspace {2mm} -;
      \enclose[rounded corners=10pt] {
         cnot c | b;
         cnot b | c;
      }
   \end{yquant}
...
\end{frame}
         ...
         \compat{New in 0.9}You can also use overlay specifications for enclosing regions: this is as simple as saying \texlink[\enclose]{\enclose<...>}, \texlink[\encloseall]{\encloseall<...>}, or \texlink[\startenclose]{\startenclose<...>}.
         Note that you cannot put an overlay specification to \texlink{\stopenclose}---it will automatically use the one from the corresponding \texlink{\startenclose}.
         By default, this overlay is enforced using \tex!\uncover!.
         You may also change this behavior; but be aware of the fact that if you use the \ttlink[/yquant/enclose/shape/rectangle]{rectangle} shape, an \ttlink{align} gate will be enforced after the enclosed region.
         Hence, using \tex!\only! for the overlay may visually alter the circuit by shifting parts.

         \begin{option}{enclose/overlay command}!\uncover!
            This \pkg{beamer} overlay command is used whenever \texlink{\enclose}, \texlink{\encloseall} or \texlink{\startenclose} is invoked with an overlay specification.
            Note that this key cannot be changed using the implicit options given in square brackets after the macros; it has to be changed before the macro invocation to have any effect.
         \end{option}
   ...
   \section{Changelog}
      ...
      \subsection*{2026-01-03: Version 0.9}
         Bugfix: Remove some legacy code that modified \TikZ{} decorations, which has not been used since version~0.1.2 and might cause some issues with other packages (problems with \pkg{pgfplots} and axis discontinuities were encountered).
         Fix \href{https://github.com/projekter/yquant/issues/35}{\#35}: Make sure to always advance all involved registers in a subcircuit, including controls. \\
         Bugfix: Controlled subcircuits will no longer potentially trigger an internal error. \\
         New: \texlink{\enclose}\hyp like macros for conveniently highlighting parts of a circuit. \\
         Bugfix: Subcircuits will now work properly even in the presence of global rotation transformations. \\
         Bugfix: Subcircuits will in vertical mode now properly account for their box space when calculating register positions.

      \subsection*{2026-04-11: Version 0.9.1}
         Fix \href{https://github.com/projekter/yquant/issues/36}{\#36}: Largely refactor the code as it appeared that some user code can be executed in internal groups which might mess with macros or registers.
%END_FOLD
\end{document}