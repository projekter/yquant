...
\usepackage{amsmath,mdframed,adjustbox,cleveref}
% END_FOLD
...
\begin{document}
   ...
   \section{Introduction}
      ...
      This alpha version~0.1 \emph{should} be stable and interfaces are not very likely to change in an incompatible way in the future.
...
\end{tikzpicture}
      ...
      \subsection{Starred vs. unstarred environment}
         ...
         However, this only works if the vector register is the last one currently in use; all registers above the last one are ``sealed,'' i.e., they cannot be enlarged any more.
      ...
      \subsection{Formal syntax}
         ...
         \begin{minted}{ebnf}
Command = { Arguments }, ?command?, [ Value ] Register, Controls, ";";
Arguments = "[", ?pgfkeys?, "]";
Value = "{", ?TeX code?, "}";
Register = RegisterSingle | RegisterMulti, [ ",", Register ];
RegisterSingle = RegisterName | RegisterRange;
RegisterMulti = "(", RegisterSingle, ")";
RegisterName = ?name?, [ "[", RegisterIndex, "]" ];
RegisterRange = [ RegisterName ], "-", [ RegisterName ];
...
\PYG{n+ni}{?name?}
         ...
         \Yquant{} does not pose particular restrictions on register names, but they should not contain any of the control literals used before (semicolon, comma, parentheses, square brackets, dash, pipe, tilde); and you should avoid using special \TeX{} characters.
         Valid values for
         \begin{BVerbatim}[commandchars=\\\{\}]
\PYG{n+ni}{?command?}
         \end{BVerbatim}
         \space (case\hyp insensitive) are documented in a later section.
      ...
      \subsection{Registers}
         ...
         Both are inclusive.
         In a range specifier, omitting the start name means that the range begins at the first known qubit; omitting the end name means that the range ends at the last known (at the moment of its use) qubit.
         Omitting both indicates a range over all known registers.
         
         Note that all those specifications refer to the operation being carried out on each of the registers \emph{individually}, i.e., producing several copies of the operation.
         ...
         Consequently it is forbidden (in the sense of ``not useful,'' but \Yquant{} does not check for this) to list the same register multiple times in one operation.
         ...
         However, note that some gates, such as the \gate{swap} gate, always require multi\hyp register operations with a fixed number of constituents; others, such as the \gate{slash} pseudo\hyp gate always require single\hyp register operations.
         Again others are completely flexible.
         \Yquant{} will prevent you from using a gate in a multi\hyp qubit setting when it may only be used for single registers simply by no longer recognizing the parenthesis---this check comes almost for free.
...
\end{tikzpicture}
            ...
            Other alternatives to the declaration would be to use three different registers that can be assigned a text individually.
         \end{example}
...
\end{tikzpicture}
            \end{codeexample}
            Observe that here we chose the simplest solution and used three scalar registers, as the classical one is in between the quantum ones.
         \end{example}
...
\end{tikzpicture}
            ...
            Instead of \tex!(q)!, we could have also written, e.g., \tex!(q[0]-q[2])! or enumerated all sub\hyp registers in a comma\hyp separated list.
         \end{example}
...
\begin{tikzpicture}
   \begin{yquant}
      qubit {$\ket{q_{\idx}} = \ifcase\idx \ket\psi \else \ket+ \fi$} q[2];
...
\begin{tikzpicture}
   \begin{yquant}
      qubit {$\ket{q_{\idx}} = \ket{\ifcase\idx\relax \psi \or \psi \or \phi \or 0 \fi}$} q[4];
...
\end{tikzpicture}
            \end{codeexample}
            This non\hyp standard gate is something that would have to be defined in an accompanying text; probably it is a generalization of \tex!zz!, $\symbb1 - 2 \ketbra{1\dotsm1}{1\dotsm1}$.
            We implemented it by carrying out a phase gate (which has a mandatory \texttt{value} attribute, which we left empty) and conditioned it on the rest.
         \end{example}
      \endgroup
...
\end{tikzpicture}
               ...
               But this would then have prevented us from extending the syndrome register.
   ...
   \begin{yquant*}
      {
         ...
         init {$\ket{0^k}$} (a[0],a[1]);
      }
      init {$\ket\psi$} (b[0],b[1]);
...
\begin{tikzpicture}
   \begin{yquant*}
      box {$\symcal F$} (a[0]-a[1]);
      [name=g]
      box {$\symcal G$} a[0], a[2];
      \draw (g-0) -- (g-1);
   \end{yquant*}
\end{tikzpicture}
               \end{codeexample}
...
\begin{tikzpicture}
   \begin{yquant*}
      measure {$\ket{\xi_\mp}$} (a[0]-a[1],a[3]);
   \end{yquant*}
\end{tikzpicture}
                  \end{codeexample}
...
\begin{tikzpicture}
   \begin{yquant*}
      init {$\ket0^{\otimes3}$} (a[0]-a[2]);
      
      box {$U$} (a[1]-a[2]) | a[0];
      measure a[0];
      discard a[0];
      output {$\ket{11}$} (a[1]-a[2]);
   \end{yquant*}
\end{tikzpicture}
               \end{codeexample}
...
\end{tikzpicture}
                  \end{codeexample*}
                  Since the ``thin'' style is the default, we here present the opposite.
                  Also note that by default, all operators are transparent, here we changed this by giving all of them a white background color (but as a style \emph{prefix}, so that, e.g., black fillings overwrite this).
                  Contrary to \pkg{quantikz}, this also fills the \gate{cnot}s.
                  If you only want to fill certain operators, you have to selectively target them using their appropriate styles.
               \end{example}
...
\end{tikzpicture}
   ...
   \section{Wishlist}
      ...
      \begin{itemize}
         ...
         \item Subranges. \\
            It would be nice to also have support for ranges within vector registers, such as \yquant!a[1-3]! or \yquant!a[-5]!.
         \item Whitespaces in ranges. \\
            At the moment, whitespace handling in range specifiers is somewhat restricted.
            Better not to use whitespaces.
