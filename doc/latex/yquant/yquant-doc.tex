...
\end{tikzpicture}
      ...
      \subsection[\texorpdfstring{\pkg{qpic}}{qpic} documentation]{\compat{New in 0.6}\pkg{qpic} documentation}
      \begingroup
         \yquantset{operator/separation=3mm}%
         Again, our section headings will be the same as in the \pkg{qpic} manual.
         As the \pkg{qpic} manual uses a bit larger separation between the operators than \Yquant's default, we globally say\\*\tex!\yquantset{operator/separation=3mm}!.
...
\begin{tikzpicture}
   \begin{yquant}
      ...
      box {\rotatebox{90}{$\operatorname{MAJ}$}} (-);
...
\end{tikzpicture}
            ...
            \paragraph{2.2 Example 2: Quantum Fourier Transform}
               \begin{example}
                  \begin{codeexample*}
\begin{tikzpicture}
   \begin{yquant}[operators/every box/.append style={shape=yquant-circle, radius=2.5mm}, operators/every h/.append style={shape=yquant-rectangle}]
      qubit {$x_2$} x2;
      qubit {$x_1$} x1;
      qubit {$x_0$} x0;
